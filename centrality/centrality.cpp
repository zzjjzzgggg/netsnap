...
void rank_undirected_graph(const TStr fi, const TStr fo, const TStr calcs){
	...
	for (TUNGraph::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
		const int NId = NI.GetId();
		fprintf(F, "%d", NId);
		for(int i=0; i<kvals.Len(); i++) fprintf(F, "\t%.6f", kvals[i](NId).Val);
		fprintf(F, "\n");
	}
	fclose(F);
}
...
void rank_graph_core(const TStr InFNm, const TStr OutFNm, const bool isdir=false) {
	if(!isdir){
		...
		while(KCore.GetNextCore()!=0){
			...
			for(int i=0; i<nodes.Len(); i++) fprintf(F, "%d\t%d\n", nodes[i].Val, core);
		}
		fclose(F);
	}else{
...
void rank_directed_graph(const TStr InFNm, const TStr OutFNm){
	...
	fprintf(F,"#NodeId\tInDegree\tOutDegree\tCloseness\tBetweennes\tEigenVector\tClustering\tPageRank\tHub\tAuthority\n");
	for (TUNGraph::TNodeI NI = UGraph->BegNI(); NI < UGraph->EndNI(); NI++) {
		const int NId = NI.GetId();
...
int main(int argc, char* argv[]) {
	...
	Try
		if(IsDir) rank_directed_graph(InFNm, OutFNm, calcs);
		else rank_undirected_graph(InFNm, OutFNm, calcs);
	CatchAll
	return 0;
}
