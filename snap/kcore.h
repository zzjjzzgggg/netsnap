...
public:
  ...
  const TIntV& GetCoreV() const {return DelV; } // nodes deleted in this round
...
int TKCore<PGraph>::GetCoreEdges() const {
	int CoreEdges = 0;
	for (int k = DegH.FFirstKeyId(); DegH.FNextKeyId(k); ) CoreEdges += DegH[k];
	return CoreEdges/2;
}
