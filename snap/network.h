...
public:
	...
	void Clr(const bool& DoDel=true, const bool& ResetDat=true) {MxNId = 0;	NodeH.Clr(DoDel, -1, ResetDat);}
...
public:
	...
	bool HasFlag(const TGraphFlag& Flag) const;
	TNodeEDatNet& operator = (const TNodeEDatNet& NodeNet) {if(this!=&NodeNet) {NodeH=NodeNet.NodeH; MxNId=NodeNet.MxNId;}	return *this;}
	// nodes
	int GetNodes() const {return NodeH.Len();}
	int AddNode(int NId = -1);
	int AddNode(int NId, const TNodeData& NodeDat);
...
int TNodeEDatNet<TNodeData, TEdgeData>::AddNode(int NId, const TNodeData& NodeDat) {
	if(NId == -1) {NId = MxNId;	MxNId++;}
	else if(IsNode(NId)) {	NodeH.GetDat(NId).NodeDat=NodeDat;	return NId;} // already a node
	else {MxNId = TMath::Mx(NId+1, MxNId());}
	IAssertR(!IsNode(NId), TStr::Fmt("NodeId %d already exists", NId));
	NodeH.AddDat(NId, TNode(NId, NodeDat));
	return NId;
...
public:
	...
	public:
		...
		TNodeData& GetInNDat(const int& EdgeN) {return Net->GetNDat(GetInNId(EdgeN));} //ori: GetNodeDat
