...
struct TGetSubGraph {
	static PGraph Do(const PGraph& Graph, const TIntV& NIdV) {
		...
		for (int n = 0; n < NIdV.Len(); n++) NewGraph.AddNode(Graph->GetNI(NIdV[n]));
		for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
			if (NewGraph.IsNode(EI.GetSrcNId()) && NewGraph.IsNode(EI.GetDstNId())) NewGraph.AddEdge(EI);
		}
		NewGraph.Defrag();
		return NewGraphPt;
	}
...
struct TGetSubGraph<PGraph, false> { // not multigraph
	static PGraph Do(const PGraph& Graph, const TIntV& NIdV) {
		...
		for (int n = 0; n < NIdV.Len(); n++) {
			if (! HasGraphFlag(typename PGraph::TObj, gfNodeDat)) NewGraph.AddNode(NIdV[n]);
			else NewGraph.AddNode(Graph->GetNI(NIdV[n]));
		}
		for (int n = 0; n < NIdV.Len(); n++) {
			...
			for (int edge = 0; edge < NI.GetOutDeg(); edge++) {
				...
				if (NewGraph.IsNode(OutNId)) {
					if (! HasGraphFlag(typename PGraph::TObj, gfEdgeDat)) NewGraph.AddEdge(SrcNId, OutNId);
					else NewGraph.AddEdge(Graph->GetEI(SrcNId, OutNId));
				}
			}
		}
		NewGraph.Defrag();
...
PGraph GetESubGraph(const PGraph& Graph, const TIntV& EIdV) {
	...
	for (int edge = 0; edge < EIdV.Len(); edge++) {
		...
		if (!NewGraph.IsNode(EI.GetSrcNId())) NewGraph.AddNode(Graph->GetNI(EI.GetSrcNId()));
		if (!NewGraph.IsNode(EI.GetDstNId())) NewGraph.AddNode(Graph->GetNI(EI.GetDstNId()));
		NewGraph.AddEdge(EI);
	}
	return NewGraphPt;
}
...
PGraph GetEDatSubGraph(const PGraph& Graph, const TEdgeDat& EDat,	const int& Cmp) {
	...
	typename PGraph::TObj& NewGraph = *NewGraphPt;
	for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
		if ((Cmp == 1 && EI() > EDat) || (Cmp == -1 && EI() < EDat) || (Cmp
				== 0 && EI() == EDat)) {
...
PGraph GetEDatSubGraph(const PGraph& Graph, const TIntV& NIdV,	const TEdgeDat& EDat, const int& Cmp) {
	...
	for (int n = 0; n < NIdV.Len(); n++) NewGraph.AddNode(Graph->GetNI(NIdV[n]));
	for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
		if ((NewGraph.IsNode(EI.GetSrcNId()) && NewGraph.IsNode(EI.GetDstNId()) && (Cmp == 1 && EI()>EDat))
			|| (Cmp==-1 && EI()<EDat) || (Cmp==0 && EI()==EDat))
			NewGraph.AddEdge(EI);
	}
	NewGraph.Defrag();
	return NewGraphPt;
...
POutGraph ConvertGraph(const PInGraph& InGraph, const bool& RenumberNodes) {
	typedef typename PInGraph::TObj::TNodeI TNodeI;
	typedef typename PInGraph::TObj::TEdgeI TEdgeI;
	...
	if (!RenumberNodes) {
		for (TNodeI NI = InGraph->BegNI(); NI < InGraph->EndNI(); NI++) OutGraph.AddNode(NI.GetId());
		for (TEdgeI EI = InGraph->BegEI(); EI < InGraph->EndEI(); EI++) {
		...
		for (TNodeI NI = InGraph->BegNI(); NI<InGraph->EndNI(); NI++) {
			const int nid = NIdSet.AddKey(NI.GetId());
			OutGraph.AddNode(nid);
		}
		for (TEdgeI EI = InGraph->BegEI(); EI < InGraph->EndEI(); EI++) {
			const int SrcNId = NIdSet.GetKeyId(EI.GetSrcNId()), DstNId = NIdSet.GetKeyId(EI.GetDstNId());
...
struct TConvertSubGraph {
	static POutGraph Do(const PInGraph& InGraph, const TIntV& NIdV, const bool& RenumberNodes) {
		...
		typename POutGraph::TObj& OutGraph = *OutGraphPt;
		if (!RenumberNodes) {
			for (int n = 0; n < NIdV.Len(); n++) OutGraph.AddNode(NIdV[n]);
			...
			for (typename PInGraph::TObj::TEdgeI EI = InGraph->BegEI(); EI
					< InGraph->EndEI(); EI++) {
...
POutGraph ConvertESubGraph(const PInGraph& InGraph, const TIntV& EIdV, const bool& RenumberNodes) {
	...
	if (!RenumberNodes) {
		for (int edge = 0; edge < EIdV.Len(); edge++) {
			...
			if (!NewGraph.IsNode(SrcNId)) NewGraph.AddNode(SrcNId);
			if (!NewGraph.IsNode(DstNId)) NewGraph.AddNode(DstNId);
...
PGraph GetRndESubGraph(const PGraph& Graph, const int& NEdges) {
	...
	for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) EdgeV.Add(TIntPr(EI.GetSrcNId(), EI.GetDstNId()));
