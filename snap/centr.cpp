...
void GetBetweennessCentr(const PUNGraph& Graph, const TIntV& BtwNIdV,
		TIntFltH& NodeBtwH, const bool& DoNodeCent, TIntPrFltH& EdgeBtwH, const bool& DoEdgeCent) {
...
void GetEigenVectorCentr(const PUNGraph& Graph, TIntFltH& EigenH, const double& Eps, const int& MaxIter) {
	...
	EigenH.Gen(NNodes);
	for (TUNGraph::TNodeI NI=Graph->BegNI(); NI<Graph->EndNI(); NI++)	EigenH.AddDat(NI.GetId(), 1.0 / NNodes);
	TFltV TmpV(NNodes);
	double diff = TFlt::Mx;
	for (int iter=0; iter<MaxIter; iter++) {
		...
		for (TUNGraph::TNodeI NI=Graph->BegNI(); NI<Graph->EndNI(); NI++, j++) {
			TmpV[j] = 0;
			for (int e = 0; e<NI.GetInDeg(); e++) TmpV[j] += EigenH.GetDat(NI.GetInNId(e));
		}
		double sum = 0;
		...
		for (int i=0; i<EigenH.Len(); i++) 	EigenH[i] /= sum;
