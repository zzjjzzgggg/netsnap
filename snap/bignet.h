...
public:
	...
	public:
		...
		TNode(const int& InVecId, const int& OutVecId, const TNodeDat& NodeDat) : InVId(InVecId), OutVId(OutVecId), Dat(NodeDat) {}
	...
	public:
		...
		TNodeData& GetDstNDat() {	return EndNode.GetDat();}
...
public:
	...
	static PBigNet New(const int& Nodes, const TSize& Edges, const bool& Sources = false) {return PBigNet(new TBigNet(Nodes, Edges, Sources));}
	static PBigNet Load(TSIn& SIn) {return PBigNet(new TBigNet(SIn));}
	TBigNet& operator =(const TBigNet& Net) {if (this != &Net) {MxNId = Net.MxNId; Flags = Net.Flags; Pool = Net.Pool; NodeH = Net.NodeH;} return *this;}

	bool OnlySources() const {return Flags.In(gfSources);}
	bool HasFlag(const TGraphFlag& Flag) const {return	HasGraphFlag(typename TBigNet, Flag) || (Flag==gfSources && OnlySources());}
	...
	// edges
	::TSize GetEdges() const {return Pool.GetVals()/2;}
...
inline void TBigNet<TNodeData, IsDir>::TNodeI::GetInOutNIdV() {
	...
	if (!Pool->IsVId(N.InVId)) InDeg = OutDeg = 0;
	else {
...
template<class TNodeData, bool IsDir>
TBigNet<TNodeData, IsDir>::TBigNet(const int& Nodes, const TSize& Edges, const bool& Sources) : CRef(), MxNId(0), Flags(), Pool(IsDir ? 2*Edges : Edges, 10000000, true, TInt::Mx), NodeH(Nodes) {
	if (Sources) {
...
void TBigNet<TNodeData, IsDir>::SetInNIdV(int NId, const TIntV& InNIdV) {
	TNode Node;
	EAssert(IsNode(NId, Node));
	TIntV InNodesV;
	Pool.GetV(Node.InVId, InNodesV);
...
void TBigNet<TNodeData, IsDir>::SetOutNIdV(int NId, const TIntV& OutNIdV) {
	...
	EAssert(IsNode(NId, Node));
	TIntV OutNodesV;
	Pool.GetV(Node.OutVId, OutNodesV);
...
void TBigNet<TNodeData, IsDir>::GetInNIdV(int NId, TIntV& InNIdV) const {
	TNode Node;
	EAssertR(IsNode(NId, Node), TStr::Fmt("Not node: NId=%d\n", NId).CStr());
...
bool TBigNet<TNodeData, IsDir>::IsEdge(const int& SrcNId, const int& DstNId, const bool& Dir) const {
	TNode Src, Dst;
	if (!IsNode(SrcNId, Src)) return false;
	// no source node
	...
	if (!IsDstNode) return false;
	// no destination node
	else if (IsEdge) return true;
	// destination and link found
...
void TBigNet<TNodeData, IsDir>::InvertFromSources(uint ExpectNodes) {
	...
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++, c++) {
		for (int e = 0; e < NI.GetOutDeg(); e++) InDegH.AddDat(NI.GetOutNId(e)) += 1;
	...
	if (2 * GetEdges() > Pool.Reserved()) Pool.Reserve(2 * GetEdges());
	// add nodes
	printf("NodeH: %s nodes, InDeg: %s nodes, Reserve: %s\n",
			TInt::GetMegaStr(NodeH.Len()).CStr(),
			TInt::GetMegaStr(InDegH.Len()).CStr(),
			TInt::GetMegaStr(ExpectNodes).CStr());
	...
	for (TDegHash::TIter I = InDegH.BegI(); I < InDegH.EndI(); I++) {
		...
		if (!IsNode(NId)) {
			AddNode(NId, InDeg, 0);
			NDest++;
		} // new destination node, no out-links
		else {
	...
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++, c++) NIdToPtH.AddDat(NI.GetId(), (int *) Pool.GetValVPt(NI.GetInVId()));
	// add in-edges
	printf("Adding edges...\n");
	c = 0;
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++, c++) {
...
void TBigNet<TNodeData, IsDir>::Rewire(TRnd& Rnd) {
	...
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++, cnt++) {
		...
		for (int i = 0; i < Len - 1 && V[i] != DelNId; i++) {
			if (V[i] == V[i + 1] || V[i] == NI.GetId()) {
				memcpy(V + i, V + i + 1, sizeof(int) * (Len - i - 1));
				i--;
				V[Len - 1] = DelNId;
				NDup++;
			}
		}
		if (Len > 0 && V[Len - 1] == NI.GetId()) {
			V[Len - 1] = DelNId;
			NDup++;
		}
		if (cnt % Mega(10) == 0) {
	...
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++, cnt++) {
		for (int e = 0; e < NI.GetOutDeg(); e++) {
			...
			const int InVLen = Pool.GetVLen(InVId);
			IAssert(InVLen >= 0 && InVLen < Mega(3));
			int* InV = (int *) Pool.GetValVPt(InVId);
			int* Val = (int *) BinSearch2(InV, InV + InVLen, NI.GetId());
			if (*Val == NI.GetId()) continue;
			// points back, everything is ok
			if (InVLen > 0 && InV[InVLen - 1] == DelNId) { // there is space at the end, insert
				...
				*Val = NI.GetId();
				...
				NI.OutNIdV[NI.GetOutDeg() - 1] = DelNId;
				e--;
			}
			NResolve++;
		}
		...
		for (int NTries = 0; NTries < 4 * NI1Slots && NI.GetOutNId(NI.GetOutDeg() - 1) == DelNId; NTries++) {
			...
			if (NI2.GetOutNId(NI2.GetOutDeg() - 1) == DelNId && !NI2.IsInNId(NI.GetId())) {
				...
				*V2 = NI.GetId();
				NAddit++;
				SlotNIdH.GetDat(nid)--;
				SlotNIdH.GetDat(nid2)--;
			}
			if(SlotNIdH.GetDat(nid2) == 0) {
				SlotNIdH.DelKey(nid2);
				continue;
			}
			if (SlotNIdH.GetDat(nid) == 0) {
				SlotNIdH.DelKey(nid);
				break;
			}
		}
...
PNGraph TBigNet<TNodeData, IsDir>::GetSubNGraph(const TIntV& NIdV) const {
	...
	// add nodes
	for (int i = 0; i < NIdV.Len(); i++) Graph->AddNode(NIdV[i]);
	...
	for (int i = 0; i < NIdV.Len(); i++) {
		...
		for (int e = 0; e < NI.GetInDeg(); e++) {
			if (Graph->IsNode(NI.GetInNId(e))) InDeg++;
		}
		for (int e = 0; e < NI.GetOutDeg(); e++) {
			if (Graph->IsNode(NI.GetOutNId(e))) OutDeg++;
		}
	...
	for (int i = 0; i < NIdV.Len(); i++) {
		...
		for (int e = 0; e < NI.GetOutDeg(); e++) {
			...
			if (Graph->IsNode(DstNId)) Graph->AddEdge(SrcNId, DstNId);
		}
	}
	return Graph;
}
...
TPt<TBigNet<TNodeData, IsDir> > TBigNet<TNodeData, IsDir>::GetSubGraph(const TIntV& NIdV, const bool& RenumberNodes) const {
	...
	for (int i = 0; i < NIdV.Len(); i++) {
		...
		int InDeg = 0, OutDeg = 0;
		for (int n = 0; n < NI.GetOutDeg(); n++) if (NIdDegH.IsKey(NI.GetOutNId(n))) OutDeg++;
		if (!onlySources && isDir) {
			for (int n = 0; n < NI.GetInDeg(); n++) if (NIdDegH.IsKey(NI.GetInNId(n))) InDeg++;
		}
		Edges += OutDeg;
	...
	if (isDir || onlySources) {
		for (int i = 0; i < NIdV.Len(); i++) {
			...
			NewNet.AddNode(NIdV[i], Deg.Val1, Deg.Val2, GetNDat(NIdV[i]));
		} // also copy the node data
	} else {
		for (int i = 0; i < NIdV.Len(); i++) {
			const TIntPr& Deg = NIdDegH.GetDat(NIdV[i]);
			NewNet.AddUndirNode(NIdV[i], Deg.Val2, GetNDat(NIdV[i]));
		} // also copy the node data
	}
	// add edges
	for (int i = 0; i < NIdV.Len(); i++) {
		...
		if (isDir && !onlySources) {
			EAssert((NI.GetInVId() == NI.GetOutVId() && NI.GetInVId() == 0) || (NI.GetInVId() != NI.GetOutVId()));
...
void TBigNet<TNodeData, IsDir>::GetSubGraph(const TIntV& NIdV, TBigNet* NewNetPt, const bool& RenumberNodes) const {
	...
	if (!RenumberNodes) {
		...
		NIdMap.Gen(NIdV.Len());
		for (int i = 0; i < NIdV.Len(); i++) {
			NIdMap.AddKey(NIdV[i]);
		}
		if (isDir || onlySources) {
			for (int i = 0; i < NIdV.Len(); i++) {
				const TIntPr& Deg = NIdDegH.GetDat(NIdV[i]);
				NewNet.AddNode(NIdMap.GetKeyId(NIdV[i]), Deg.Val1, Deg.Val2, GetNDat(NIdV[i]));
			} // also copy the node data
		} else {
			for (int i = 0; i < NIdV.Len(); i++) {
				const TIntPr& Deg = NIdDegH.GetDat(NIdV[i]);
				NewNet.AddUndirNode(NIdMap.GetKeyId(NIdV[i]), Deg.Val2, GetNDat(NIdV[i]));
			} // also copy the node data
		}
	}
	// add edges
...
bool TBigNet<TNodeData, IsDir>::IsOk() const {
	...
	for (uint id = 1; id < Pool.GetVecs(); id++) {
		...
		if (!ValV.Empty()) {
			...
			try {
				for (int i = 1; i < ValV.Len(); i++) {
					...
					EAssertR((ValV[i-1] < ValV[i]) || (ValV[i-1] == ValV[i] && ValV[i] == DelNId), TStr::Fmt("nid1: %d nid2:%d", ValV[i - 1], ValV[i]));
					EAssertR((0 <= ValV[i] && ValV[i] < MxNId) || ValV[i] == DelNId, TStr::Fmt("nid1: %dm mxNId: %d", ValV[i], MxNId));
					if (!OnlySources()) EAssertR(IsNode(ValV[i]) || ValV[i] == DelNId, TStr::Fmt("nid1: %dm mxNId: %d", ValV[i]));
		...
		if (id % 10000 == 0) printf("\r  %dk / %dk [%s]", id / 1000, Pool.GetVecs() / 1000, ExeTm.GetStr());
	}
	printf("[%s]\n  Check Edges...\n", ExeTm.GetStr());
	// check edges
	int ErrCnt = 0;
	if (!OnlySources()) {
		...
		for (TNodeI NI = BegNI(); NI < EndNI(); NI++, Cnt++) {
			...
			for (int e = 0; e < NI.GetInDeg(); e++) {
				if (NI.GetInNId(e) == DelNId) continue;
				// skip deleted nodes
			...
			if (Cnt % 100000 == 0) printf("\r%s [%s]", TInt::GetMegaStr(Cnt).CStr(), ExeTm.GetStr());
		}
...
void TBigNet<TNodeData, IsDir>::SaveForDisk(const TStr& OutFNm) const {
	...
	for (TNodeI NI = BegNI(); NI < EndNI(); NI++) {
		...
		for (int i = 0; i < NI.GetOutDeg(); i++) FOut.Save(NI.GetOutNId(i));
		if (IsDirected) {
			FOut.Save(NI.GetInDeg());
			for (int i = 0; i < NI.GetInDeg(); i++) FOut.Save(NI.GetInNId(i));
		}
	}
}
...
void TBigNet<TNodeData, IsDir>::LoadNodeDatH(const TStr& InFNm, TNodeH& NodeH) {
	TFIn SIn(InFNm);	TInt MxNId(SIn);	TB32Set Flags(SIn);
	printf("skipping Pool...\n");
	TBool FastCopy(SIn);	uint64 _GrowBy, _MxVals, _Vals;
	SIn.Load(_GrowBy);	SIn.Load(_MxVals);	SIn.Load(_Vals);
	TInt EmptyVal(SIn);
	int Tmp;
	for (TSize ValN = 0; ValN < _Vals; ValN++)  SIn.Load(Tmp);
	TInt MxVals(SIn), Vals(SIn);	uint64 Offset;
...
void TBigNet<TNodeData, IsDir>::SaveToDisk(const TStr& InFNm, const TStr& OutFNm, const bool& SaveSparseHash) {
	TFIn FIn(InFNm);	TFOut FOut(OutFNm);
	...
	if (!SaveSparseHash) {
		...
		for (typename TNodeH::TIter I = NodeH.BegI(); I < NodeH.EndI(); I++)  NewH.AddDat(I->Key, I->Dat);
		NewH.Save(FOut);
