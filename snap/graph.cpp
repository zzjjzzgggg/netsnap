...
int TUNGraph::AddNode(const int& NId, const TIntV& NbhNIdV) {
    IAssert(NId != -1);
    IAssertR(!IsNode(NId),
             TStr::Fmt("NodeId %d already exists", NId));
    MxNId = TMath::Mx(NId + 1, MxNId());
...
// (use TUNGraph::IsOk to check whether the graph is consistent)
int TUNGraph::AddNode(const int& NId, const TVecPool<TInt>& Pool,
                      const int& NIdVId) {
    IAssert(NId != -1);
    IAssertR(!IsNode(NId),
             TStr::Fmt("NodeId %d already exists", NId));
...
int TUNGraph::AddEdge(const int& SrcNId, const int& DstNId) {
    IAssertR(
        ...
        TStr::Fmt("%d or %d not a node.", SrcNId, DstNId).CStr());
    // edge already exists
...
bool TUNGraph::IsOk(const bool& ThrowExcept) const {
    ...
    for (int N = NodeH.FFirstKeyId(); NodeH.FNextKeyId(N);) {
        ...
        if (!Node.NIdV.IsSorted()) {
            const TStr Msg =
                TStr::Fmt("Neighbor list of node %d is not sorted.",
                          Node.GetId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
        ...
        for (int e = 0; e < Node.GetDeg(); e++) {
            if (!IsNode(Node.GetNbhNId(e))) {
                const TStr Msg = TStr::Fmt(
                    "Edge %d --> %d: node %d does not exist.",
                    Node.GetId(), Node.GetNbhNId(e),
                    Node.GetNbhNId(e));
                if (ThrowExcept) {
                    EAssertR(false, Msg);
                } else {
...
void TUNGraph::Dump(FILE* OutF) const {
    ...
    for (int N = NodeH.FFirstKeyId(); NodeH.FNextKeyId(N);) {
        const TNode& Node = NodeH[N];
        fprintf(OutF, "	%*d [%d] ", NodePlaces, Node.GetId(),
                Node.GetDeg());
...
int TNGraph::AddNode(const int& NId, const TIntV& InNIdV,
                     const TIntV& OutNIdV) {
    IAssert(NId != -1);
    IAssertR(!IsNode(NId),
             TStr::Fmt("NodeId %d already exists", NId));
...
// add a node from a vector pool
// (use TNGraph::IsOk to check whether the graph is consistent)
int TNGraph::AddNode(const int& NId, const TVecPool<TInt>& Pool,
    ...
    Node.OutNIdV.GenExt(Pool.GetValVPt(DstVId),
                        Pool.GetVLen(DstVId));
    Node.InNIdV.Sort();
    Node.OutNIdV.Sort();
    return NId;
...
void TNGraph::DelEdge(const int& SrcNId, const int& DstNId,
                      const bool& Dir) {
    IAssertR(
        IsNode(SrcNId) && IsNode(DstNId),
        TStr::Fmt("%d or %d not a node.", SrcNId, DstNId).CStr());
...
bool TNGraph::IsEdge(const int& SrcNId, const int& DstNId,
...
bool TNGraph::IsOk(const bool& ThrowExcept) const {
    ...
    for (int N = NodeH.FFirstKeyId(); NodeH.FNextKeyId(N);) {
        ...
        if (!Node.OutNIdV.IsSorted()) {
            const TStr Msg = TStr::Fmt(
                "Out-neighbor list of node %d is not sorted.",
                Node.GetId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
        ...
        if (!Node.InNIdV.IsSorted()) {
            const TStr Msg = TStr::Fmt(
                "In-neighbor list of node %d is not sorted.",
                Node.GetId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
...
int TNEGraph::AddEdge(const int& SrcNId, const int& DstNId,
                      int EId) {
    if (EId == -1) {
        EId = MxEId;
        MxEId++;
...
void TNEGraph::DelEdge(const int& SrcNId, const int& DstNId,
                       const bool& Dir) {
    int EId;
    IAssert(IsEdge(SrcNId, DstNId, EId,
                   Dir));  // there is at least one edge
...
bool TNEGraph::IsEdge(const int& SrcNId, const int& DstNId, int& EId,
                      const bool& Dir) const {
...
void TNEGraph::Defrag(const bool& OnlyNodeLinks) {
    ...
    if (!OnlyNodeLinks && !EdgeH.IsKeyIdEqKeyN()) {
        EdgeH.Defrag();
    }
}
...
bool TNEGraph::IsOk(const bool& ThrowExcept) const {
    ...
    for (int N = NodeH.FFirstKeyId(); NodeH.FNextKeyId(N);) {
        ...
        if (!Node.OutEIdV.IsSorted()) {
            const TStr Msg =
                TStr::Fmt("Out-edge list of node %d is not sorted.",
                          Node.GetId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
        ...
        if (!Node.InEIdV.IsSorted()) {
            const TStr Msg =
                TStr::Fmt("In-edge list of node %d is not sorted.",
                          Node.GetId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
        ...
        for (int e = 0; e < Node.GetOutDeg(); e++) {
            if (!IsEdge(Node.GetOutEId(e))) {
                const TStr Msg = TStr::Fmt(
                    "Out-edge id %d of node %d does not exist.",
                    Node.GetOutEId(e), Node.GetId());
                if (ThrowExcept) {
                    EAssertR(false, Msg);
                } else {
                    ErrNotify(Msg.CStr());
            ...
            if (e > 0 && prevEId == Node.GetOutEId(e)) {
                const TStr Msg = TStr::Fmt(
                    "Node %d has duplidate out-edge id %d.",
                    Node.GetId(), Node.GetOutEId(e));
                if (ThrowExcept) {
                    EAssertR(false, Msg);
                } else {
                    ErrNotify(Msg.CStr());
    ...
    for (int E = EdgeH.FFirstKeyId(); EdgeH.FNextKeyId(E);) {
        ...
        if (!IsNode(Edge.GetSrcNId())) {
            const TStr Msg =
                TStr::Fmt("Edge %d source node %d does not exist.",
                          Edge.GetId(), Edge.GetSrcNId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
                ErrNotify(Msg.CStr());
        ...
        if (!IsNode(Edge.GetDstNId())) {
            const TStr Msg = TStr::Fmt(
                "Edge %d destination node %d does not exist.",
                Edge.GetId(), Edge.GetDstNId());
            if (ThrowExcept) {
                EAssertR(false, Msg);
            } else {
                ErrNotify(Msg.CStr());
...
void TNEGraph::Dump(FILE* OutF) const {
    ...
    for (TEdgeI EdgeI = BegEI(); EdgeI < EndEI(); EdgeI++) {
        fprintf(OutF, "	%*d]	%*d	->	%*d\n", EdgePlaces,
                EdgeI.GetId(), NodePlaces, EdgeI.GetSrcNId(),
                NodePlaces, EdgeI.GetDstNId());
    }
    fprintf(OutF, "\n");
}
...
int TBNEGraph::AddSrcNode(int NId) {
    if (NId == -1) {
        NId = MxSrcNId;
        MxSrcNId++;
    } else if (IsSrcNode(NId)) {
        return NId;
    }  // already a node
    else {
        MxSrcNId = TMath::Mx(NId + 1, MxSrcNId());
    }
...
int TBNEGraph::AddDstNode(int NId) {
    if (NId == -1) {
        NId = MxDstNId;
        MxDstNId++;
    } else if (IsDstNode(NId)) {
        return NId;
    }  // already a node
    else {
        MxDstNId = TMath::Mx(NId + 1, MxDstNId());
    }
...
int TBNEGraph::AddEdge(const int& SrcNId, const int& DstNId,
                       const int& Dat, int EId) {
    if (EId == -1) {
        EId = MxEId;
        MxEId++;
    } else {
    ...
    IAssert(!IsEdge(EId));
    IAssertR(IsSrcNode(SrcNId) && IsDstNode(DstNId),
...
void TBNEGraph::DelEdge(const int& SrcNId, const int& DstNId) {
    int EId;
    IAssert(IsEdge(SrcNId, DstNId, EId)); // at least one edge exists
...
bool TBNEGraph::IsEdge(const int& SrcNId, const int& DstNId,
                       int& EId) const {
...
void TBNEGraph::Defrag(const bool& OnlyNodeLinks) {
    for (int kid = SrcNodeH.FFirstKeyId(); SrcNodeH.FNextKeyId(kid);)
        SrcNodeH[kid].EIdV.Pack();
    for (int kid = DstNodeH.FFirstKeyId(); DstNodeH.FNextKeyId(kid);)
        DstNodeH[kid].EIdV.Pack();
    if (!OnlyNodeLinks && !SrcNodeH.IsKeyIdEqKeyN()) {
        SrcNodeH.Defrag();
    }
    if (!OnlyNodeLinks && !DstNodeH.IsKeyIdEqKeyN()) {
        DstNodeH.Defrag();
    }
    if (!OnlyNodeLinks && !EdgeH.IsKeyIdEqKeyN()) {
        EdgeH.Defrag();
...
bool TBNEGraph::IsOk(const bool& ThrowExcept) const {
    ...
    for (int N = SrcNodeH.FFirstKeyId(); SrcNodeH.FNextKeyId(N);) {
        ...
        if (!Node.EIdV.IsSorted()) {
            const TStr Msg =
                TStr::Fmt("Edge list of src node %d is not sorted.",
                          Node.GetId());
        ...
        for (int e = 0; e < Node.GetDeg(); e++) {
            if (!IsEdge(Node.GetEId(e))) {
                const TStr Msg = TStr::Fmt(
                    "Edge id %d of src node %d does not exist.",
                    Node.GetEId(e), Node.GetId());
                if (ThrowExcept)
                    EAssertR(false, Msg);
                else
                    ErrNotify(Msg.CStr());
            ...
            if (e > 0 && prevEId == Node.GetEId(e)) {
                const TStr Msg = TStr::Fmt(
                    "Src node %d has duplidate edge id %d.",
                    Node.GetId(), Node.GetEId(e));
                if (ThrowExcept)
                    EAssertR(false, Msg);
                else
                    ErrNotify(Msg.CStr());
    ...
    for (int N = DstNodeH.FFirstKeyId(); DstNodeH.FNextKeyId(N);) {
        ...
        if (!Node.EIdV.IsSorted()) {
            const TStr Msg =
                TStr::Fmt("Edge list of dst node %d is not sorted.",
                          Node.GetId());
            if (ThrowExcept)
                EAssertR(false, Msg);
            else
        ...
        for (int e = 0; e < Node.GetDeg(); e++) {
            if (!IsEdge(Node.GetEId(e))) {
                const TStr Msg = TStr::Fmt(
                    "Edge id %d of dst node %d does not exist.",
                    Node.GetOutEId(e), Node.GetId());
                if (ThrowExcept)
                    EAssertR(false, Msg);
                else
            ...
            if (e > 0 && prevEId == Node.GetEId(e)) {
                const TStr Msg = TStr::Fmt(
                    "Dst node %d has duplidate edge id %d.",
                    Node.GetId(), Node.GetEId(e));
                if (ThrowExcept)
                    EAssertR(false, Msg);
                else
...
void TBNEGraph::Dump(FILE* OutF) const {
    ...
    fprintf(OutF,
            "-------------------------------------------------"
            "\nDirected Bipartite Node-Edge Graph: src-nodes: %d, "
            "dst-nodes: %d, nodes: %d, edges: %d\n",
    ...
    for (TEdgeI EdgeI = BegEI(); EdgeI < EndEI(); EdgeI++) {
        fprintf(OutF, "	%*d]	%*d	->	%*d (%d)\n",
                EdgePlaces, EdgeI.GetId(), NodePlaces,
                EdgeI.GetSrcNId(), NodePlaces, EdgeI.GetDstNId(),
                EdgeI.GetDat());
    }
    fprintf(OutF, "\n");
}
