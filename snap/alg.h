...
template <class PGraph> void GetJotDegCnt(const PGraph& Graph, TIntPrIntH& DegToCntH, const int MaxDeg=-1);
...
template <class PGraph>
void GetJotDegCnt(const PGraph& Graph, TIntPrIntH& DegToCntH, const int MaxDeg){
	for (typename PGraph::TObj::TNodeI NI = Graph->BegNI(); NI < Graph->EndNI(); NI++) {
		int iDeg=NI.GetInDeg(), oDeg=NI.GetOutDeg();
		if(MaxDeg>0 && (iDeg>MaxDeg || oDeg>MaxDeg)) continue;
		DegToCntH.AddDat(TIntPr(NI.GetInDeg(), NI.GetOutDeg()))++;
	}
}
...
void MakeUnDir(const PGraph& Graph) {
	...
	for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
		const int SrcNId = EI.GetSrcNId(), DstNId = EI.GetDstNId();
		if(!Graph->IsEdge(DstNId, SrcNId)) EdgeV.Add(TIntPr(DstNId, SrcNId));
	}
	for (int i = 0; i < EdgeV.Len(); i++) Graph->AddEdge(EdgeV[i].Val1, EdgeV[i].Val2);
}
...
void DelNodes(PGraph& Graph, const TIntV& NIdV) {
	for (int n = 0; n < NIdV.Len(); n++) Graph->DelNode(NIdV[n]);
}
