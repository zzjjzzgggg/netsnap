...
// Bipartite  graphs
class TBNEGraph;
typedef TPt<TBNEGraph> PBNEGraph;
...
public:
    ...
    public:
        ...
        int GetRndNbhNId(TRnd& Rnd = TInt::Rnd) const {
            return NIdV.GetRnd(Rnd);
        }
        bool IsNbhNId(const int& NId) const {
    ...
    public:
        ...
        int GetNbhNId(const int& NodeN) const {
            return NodeHI.GetDat().GetNbhNId(NodeN);
        }
        int GetRndNbhNId(TRnd& Rnd = TInt::Rnd) const {
            return NodeHI.GetDat().GetRndNbhNId(Rnd);
        }
        ...
        bool IsNbhNId(const int& NId) const {
            return NodeHI.GetDat().IsNbhNId(NId);
        }
        friend class TUNGraph;
    };
    class TEdgeI {
    ...
    public:
        ...
        TEdgeI(const TNodeI& NodeI, const TNodeI& EndNodeI,
               const int& EdgeN = 0)
            : CurNode(NodeI), EndNode(EndNodeI), CurEdge(EdgeN) {}
        TEdgeI(const TEdgeI& EdgeI)
            : CurNode(EdgeI.CurNode), EndNode(EdgeI.EndNode),
              CurEdge(EdgeI.CurEdge) {}

        TEdgeI& operator=(const TEdgeI& EdgeI) {
            if (this != &EdgeI) {
                CurNode = EdgeI.CurNode;
                EndNode = EdgeI.EndNode;
                CurEdge = EdgeI.CurEdge;
            }
            return *this;
        }
        ...
        bool operator<(const TEdgeI& EdgeI) const {
            return CurNode < EdgeI.CurNode ||
                   (CurNode == EdgeI.CurNode && CurEdge < EdgeI.CurEdge);
        }
        bool operator==(const TEdgeI& EdgeI) const {
...
public:
    ...
    explicit TUNGraph(const int& Nodes, const int& Edges) : MxNId(0) {
        Reserve(Nodes, Edges);
    }
    ...
    TEdgeI BegEI() const {
        TNodeI NI = BegNI();
        while (NI < EndNI() && NI.GetOutDeg() == 0) NI++;
        return TEdgeI(NI, EndNI());
    }
...
public:
    ...
    public:
        ...
        int GetNbhNId(const int& NodeN) const {
            return NodeN < GetOutDeg() ? GetOutNId(NodeN)
                                       : GetInNId(NodeN - GetOutDeg());
        }
        bool IsInNId(const int& NId) const {
...
public:
    ...
    explicit TNGraph(const int& Nodes, const int& Edges) : MxNId(0) {
        Reserve(Nodes, Edges);
    }
    ...
    int AddNode(const int& NId, const TVecPool<TInt>& Pool, const int& SrcVId,
                const int& DstVId);
    void DelNode(const int& NId);
    void DelNode(const TNode& NodeI) { DelNode(NodeI.GetId()); }
    bool IsNode(const int& NId) const { return NodeH.IsKey(NId); }
...
public:
    ...
    public:
        ...
        int GetNbhEId(const int& EdgeN) const {
            return EdgeN < GetOutDeg() ? GetOutEId(EdgeN)
                                       : GetInEId(EdgeN - GetOutDeg());
        }
    ...
    public:
        ...
        int GetNbhNId(const int& EdgeN) const {
            const TEdge& E = Graph->GetEdge(NodeHI.GetDat().GetNbhEId(EdgeN));
        ...
        int GetNbhEId(const int& EdgeN) const {
            return NodeHI.GetDat().GetNbhEId(EdgeN);
        }
        bool IsInEId(const int& EId) const {
        ...
        bool IsNbhEId(const int& EId) const {
...
private:
    TCRef CRef;
    TInt MxNId, MxEId;
    THash<TInt, TNode> NodeH;  // NId --> TNode
    THash<TInt, TEdge> EdgeH;  // EId --> TEdge

public:
    TNEGraph() : CRef(), MxNId(0), MxEId(0) {}
    explicit TNEGraph(const int& Nodes, const int& Edges)
        : CRef(), MxNId(0), MxEId(0) {
        Reserve(Nodes, Edges);
    }
    TNEGraph(const TNEGraph& Graph)
        : MxNId(Graph.MxNId), MxEId(Graph.MxEId), NodeH(Graph.NodeH),
          EdgeH(Graph.EdgeH) {}
    ...
    void DelEdge(const int& SrcNId, const int& DstNId,
                 const bool& Dir = true);  // deletes all edges
                                           // between the two nodes
    bool IsEdge(const int& EId) const { return EdgeH.IsKey(EId); }
    bool IsEdge(const int& SrcNId, const int& DstNId,
                const bool& Dir = true) const {
        int EId;
        return IsEdge(SrcNId, DstNId, EId, Dir);
    }
    bool IsEdge(const int& SrcNId, const int& DstNId, int& EId,
                const bool& Dir = true) const;
...
/////////////////////////////////////////////////
// Multi-Bipartite Graph, i.e., supports storing two kinds of nodes
// in one graph; supports one or more edges existed between two nodes
class TBNEGraph {
public:
...
public:
    ...
    public:
        ...
        TEdge(const int& EId, const int& SourceNId, const int& DestNId,
              const int& Data)
            : Id(EId), SrcNId(SourceNId), DstNId(DestNId), Dat(Data) {}
        TEdge(const TEdge& Edge)
            : Id(Edge.Id), SrcNId(Edge.SrcNId), DstNId(Edge.DstNId),
              Dat(Edge.Dat) {}
    ...
    public:
        TNodeI() : NodeHI(), Graph(NULL) {}
        TNodeI(const THashIter& NodeHIter, const TBNEGraph* GraphPt)
            : NodeHI(NodeHIter), Graph(GraphPt) {}
        TNodeI(const TNodeI& NodeI)
            : NodeHI(NodeI.NodeHI), Graph(NodeI.Graph) {}
        TNodeI& operator=(const TNodeI& NodeI) {
        ...
        // edges
        int GetEId(const int& EdgeN) const {
            return NodeHI.GetDat().GetEId(EdgeN);
        }
        int GetInEId(const int& EdgeN) const {
            return NodeHI.GetDat().GetInEId(EdgeN);
        }
    ...
    public:
        TEdgeI() : EdgeHI(), Graph(NULL) {}
        TEdgeI(const THashIter& EdgeHIter, const TBNEGraph* GraphPt)
            : EdgeHI(EdgeHIter), Graph(GraphPt) {}
        TEdgeI(const TEdgeI& EdgeI)
            : EdgeHI(EdgeI.EdgeHI), Graph(EdgeI.Graph) {}
        TEdgeI& operator=(const TEdgeI& EdgeI) {
...
private:
    ...
    const TNode& GetSrcNode(const int& NId) const {
        return SrcNodeH.GetDat(NId);
    }
    TNode& GetDstNode(const int& NId) { return DstNodeH.GetDat(NId); }
    const TNode& GetDstNode(const int& NId) const {
        return DstNodeH.GetDat(NId);
    }
    TEdge& GetEdge(const int& EId) { return EdgeH.GetDat(EId); }
    const TEdge& GetEdge(const int& EId) const { return EdgeH.GetDat(EId); }
...
public:
    ...
    explicit TBNEGraph(const int& SrcNodes, const int& DstNodes,
                       const int& Edges)
        : CRef(), MxSrcNId(0), MxDstNId(0), MxEId(0) {
        Reserve(SrcNodes, DstNodes, Edges);
    }
    TBNEGraph(const TBNEGraph& Graph)
        : MxSrcNId(Graph.MxSrcNId), MxDstNId(Graph.MxDstNId),
          MxEId(Graph.MxEId), SrcNodeH(Graph.SrcNodeH),
          DstNodeH(Graph.DstNodeH), EdgeH(Graph.EdgeH) {}
    TBNEGraph(TSIn& SIn)
        : MxSrcNId(SIn), MxDstNId(SIn), MxEId(SIn), SrcNodeH(SIn),
          DstNodeH(SIn), EdgeH(SIn) {}
    void Save(TSOut& SOut) const {
        MxSrcNId.Save(SOut);
        MxDstNId.Save(SOut);
        MxEId.Save(SOut);
    ...
    static PBNEGraph New(const int& SrcNodes, const int& DstNodes,
                         const int& Edges) {
        return PBNEGraph(new TBNEGraph(SrcNodes, DstNodes, Edges));
    }
    static PBNEGraph Load(TSIn& SIn) { return PBNEGraph(new TBNEGraph(SIn)); }
    bool HasFlag(const TGraphFlag& Flag) const;
    TBNEGraph& operator=(const TBNEGraph& Graph) {
        if (this != &Graph) {
            MxSrcNId = Graph.MxSrcNId;
            MxDstNId = Graph.MxDstNId;
            MxEId = Graph.MxEId;
            SrcNodeH = Graph.SrcNodeH;
            DstNodeH = Graph.DstNodeH;
            EdgeH = Graph.EdgeH;
        }
        return *this;
    }
    ...
    TNodeI GetSrcNI(const int& NId) const {
        return TNodeI(SrcNodeH.GetI(NId), this);
    }
    ...
    TNodeI GetDstNI(const int& NId) const {
        return TNodeI(DstNodeH.GetI(NId), this);
    }
    ...
    // edges
    int GetEdges() const { return EdgeH.Len(); }
    int AddEdge(const int& SrcNId, const int& DstNId, const int& Dat = -1,
                int EId = -1);
    int AddEdge(const TEdgeI& EdgeI) {
        return AddEdge(EdgeI.GetSrcNId(), EdgeI.GetDstNId(), EdgeI.GetDat(),
                       EdgeI.GetId());
    }
    void DelEdge(const int& EId);
    // deletes all edges between the two nodes
    void DelEdge(const int& SrcNId, const int& DstNId);
    bool IsEdge(const int& EId) const { return EdgeH.IsKey(EId); }
    bool IsEdge(const int& SrcNId, const int& DstNId) const {
        int EId;
        return IsEdge(SrcNId, DstNId, EId);
    }
    ...
    int GetRndSrcNId(TRnd& Rnd = TInt::Rnd) {
        return SrcNodeH.GetKey(SrcNodeH.GetRndKeyId(Rnd, 0.8));
    }
    TNodeI GetRndSrcNI(TRnd& Rnd = TInt::Rnd) {
        return GetSrcNI(GetRndSrcNId(Rnd));
    }
    int GetRndDstNId(TRnd& Rnd = TInt::Rnd) {
        return DstNodeH.GetKey(DstNodeH.GetRndKeyId(Rnd, 0.8));
    }
    TNodeI GetRndDstNI(TRnd& Rnd = TInt::Rnd) {
        return GetDstNI(GetRndDstNId(Rnd));
    }
    int GetRndEId(TRnd& Rnd = TInt::Rnd) {
        return EdgeH.GetKey(EdgeH.GetRndKeyId(Rnd, 0.8));
    }
