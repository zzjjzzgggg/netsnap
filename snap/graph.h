...
/////////////////////////////////////////////////
// Bipartite  graphs
class TBNEGraph; typedef TPt<TBNEGraph> PBNEGraph;
...
public:
	...
	TUNGraph& operator=(const TUNGraph& Graph){ if(this!=&Graph){MxNId=Graph.MxNId; NodeH=Graph.NodeH; } return *this; }
	// nodes
...
public:
	...
	bool HasFlag(const TGraphFlag& Flag) const;
	TNGraph& operator = (const TNGraph& Graph) {if(this!=&Graph) {MxNId=Graph.MxNId; NodeH=Graph.NodeH; } return *this; }
	// nodes
	int GetNodes() const { return NodeH.Len(); }
	int AddNode(int NId = -1);
	int AddNode(const TNodeI& NodeId) { return AddNode(NodeId.GetId()); }
...
public:
	...
	bool HasFlag(const TGraphFlag& Flag) const;
	TNEGraph& operator = (const TNEGraph& Graph) {if (this!=&Graph) {MxNId=Graph.MxNId; MxEId=Graph.MxEId; NodeH=Graph.NodeH; EdgeH=Graph.EdgeH; }	return *this; }
	// nodes
	int GetNodes() const { return NodeH.Len(); }
	int AddNode(int NId = -1);
	int AddNode(const TNodeI& NodeId) { return AddNode(NodeId.GetId()); }
	...
	void Reserve(const int& Nodes, const int& Edges) { if(Nodes>0) NodeH.Gen(Nodes/2);if(Edges>0) EdgeH.Gen(Edges/2); }
	void Defrag(const bool& OnlyNodeLinks=false);
	bool IsOk(const bool& ThrowExcept=true) const;
	void Dump(FILE *OutF=stdout) const;
...
class TBNEGraph {
public:
	typedef TBNEGraph TNet;
	typedef TPt<TBNEGraph> PNet;
public:
	class TNode {
	private:
		TInt Id;
	...
	public:
		TNode() : Id(-1), EIdV() {}
		TNode(const int& NId) : Id(NId), EIdV() {}
		TNode(const TNode& Node) : Id(Node.Id), EIdV(Node.EIdV) {}
		TNode(TSIn& SIn) : Id(SIn), EIdV(SIn) {}
		void Save(TSOut& SOut) const { Id.Save(SOut); EIdV.Save(SOut); }
		int GetId() const { return Id; }
		int GetDeg() const { return EIdV.Len(); }
		int GetInDeg() const { return EIdV.Len(); }
		int GetOutDeg() const { return EIdV.Len(); }
		int GetEId(const int& EdgeN) const { return EIdV[EdgeN]; }
		int GetInEId(const int& EdgeN) const { return EIdV[EdgeN]; }
		int GetOutEId(const int& EdgeN) const { return EIdV[EdgeN]; }
		int GetNbhEId(const int& EdgeN) const { return EIdV[EdgeN]; }
		bool IsEId(const int& EId) const { return EIdV.SearchBin(EId) != -1; }
		bool IsInEId(const int& EId) const { return EIdV.SearchBin(EId) != -1; }
		bool IsOutEId(const int& EId) const { return EIdV.SearchBin(EId) != -1; }
		friend class TBNEGraph;
	};
	class TEdge {
	private:
		TInt Id, SrcNId, DstNId, Dat;
	public:
		TEdge() : Id(-1), SrcNId(-1), DstNId(-1), Dat(-1) {}
		TEdge(const int& EId, const int& SourceNId, const int& DestNId, const int& Data) : Id(EId), SrcNId(SourceNId), DstNId(DestNId), Dat(Data) {}
		TEdge(const TEdge& Edge) : Id(Edge.Id), SrcNId(Edge.SrcNId), DstNId(Edge.DstNId), Dat(Edge.Dat) {}
		TEdge(TSIn& SIn) : Id(SIn), SrcNId(SIn), DstNId(SIn), Dat(SIn) {}
		void Save(TSOut& SOut) const { Id.Save(SOut); SrcNId.Save(SOut); DstNId.Save(SOut); Dat.Save(SOut); }
		int GetId() const { return Id; }
		int GetDat() const { return Dat; }
		int GetSrcNId() const { return SrcNId; }
		int GetDstNId() const { return DstNId; }
		friend class TBNEGraph;
	};
	...
	private:
		typedef THash<TInt, TNode>::TIter THashIter;
		THashIter NodeHI;
		const TBNEGraph *Graph;
	public:
		TNodeI() : NodeHI(), Graph(NULL) {}
		TNodeI(const THashIter& NodeHIter, const TBNEGraph* GraphPt) : NodeHI(NodeHIter), Graph(GraphPt) {}
		TNodeI(const TNodeI& NodeI) : NodeHI(NodeI.NodeHI), Graph(NodeI.Graph) {}
		TNodeI& operator = (const TNodeI& NodeI) { NodeHI = NodeI.NodeHI; Graph=NodeI.Graph; return *this; }
		TNodeI& operator++ (int) { NodeHI++; return *this; }
		bool operator < (const TNodeI& NodeI) const { return NodeHI < NodeI.NodeHI; }
		...
		int GetInNId(const int& EdgeN) const { return Graph->GetEdge(NodeHI.GetDat().GetInEId(EdgeN)).GetSrcNId(); }
		int GetOutNId(const int& EdgeN) const { return Graph->GetEdge(NodeHI.GetDat().GetOutEId(EdgeN)).GetDstNId(); }
		int GetNbhNId(const int& EdgeN) const { const TEdge& E = Graph->GetEdge(NodeHI.GetDat().GetNbhEId(EdgeN)); return GetId()==E.GetSrcNId() ? E.GetDstNId() : E.GetSrcNId(); }
		bool IsInNId(const int& NId) const;
		bool IsOutNId(const int& NId) const;
		bool IsNbhNId(const int& NId) const { return IsOutNId(NId) || IsInNId(NId); }
		// edges
		int GetEId(const int& EdgeN) const { return NodeHI.GetDat().GetEId(EdgeN); }
		int GetInEId(const int& EdgeN) const { return NodeHI.GetDat().GetInEId(EdgeN); }
		int GetOutEId(const int& EdgeN) const { return NodeHI.GetDat().GetOutEId(EdgeN); }
		int GetNbhEId(const int& EdgeN) const { return NodeHI.GetDat().GetNbhEId(EdgeN); }
		bool IsInEId(const int& EId) const { return NodeHI.GetDat().IsInEId(EId); }
	...
	public:
		TEdgeI() : EdgeHI(), Graph(NULL) {}
		TEdgeI(const THashIter& EdgeHIter, const TBNEGraph *GraphPt) : EdgeHI(EdgeHIter), Graph(GraphPt) {}
		TEdgeI(const TEdgeI& EdgeI) : EdgeHI(EdgeI.EdgeHI), Graph(EdgeI.Graph) {}
		TEdgeI& operator = (const TEdgeI& EdgeI) { if (this!=&EdgeI) {EdgeHI=EdgeI.EdgeHI; Graph=EdgeI.Graph; }	return *this; }
		TEdgeI& operator++ (int) {EdgeHI++; return *this; }
		bool operator < (const TEdgeI& EdgeI) const { return EdgeHI < EdgeI.EdgeHI; }
		bool operator == (const TEdgeI& EdgeI) const { return EdgeHI == EdgeI.EdgeHI; }
		int GetId() const { return EdgeHI.GetDat().GetId(); }
		int GetDat() const { return EdgeHI.GetDat().GetDat(); }
		int GetSrcNId() const { return EdgeHI.GetDat().GetSrcNId(); }
		int GetDstNId() const { return EdgeHI.GetDat().GetDstNId(); }
		friend class TBNEGraph;
	};
...
private:
	TNode& GetSrcNode(const int& NId) { return SrcNodeH.GetDat(NId); }
	const TNode& GetSrcNode(const int& NId) const { return SrcNodeH.GetDat(NId); }
	TNode& GetDstNode(const int& NId) { return DstNodeH.GetDat(NId); }
	const TNode& GetDstNode(const int& NId) const { return DstNodeH.GetDat(NId); }
	TEdge& GetEdge(const int& EId) { return EdgeH.GetDat(EId); }
	const TEdge& GetEdge(const int& EId) const { return EdgeH.GetDat(EId); }
private:
	TCRef CRef;
	TInt MxSrcNId, MxDstNId, MxEId;
	THash<TInt, TNode> SrcNodeH, DstNodeH;
	THash<TInt, TEdge> EdgeH;
public:
	TBNEGraph() : CRef(), MxSrcNId(0), MxDstNId(0), MxEId(0) {}
	explicit TBNEGraph(const int& SrcNodes, const int& DstNodes, const int& Edges) : CRef(), MxSrcNId(0), MxDstNId(0), MxEId(0) {Reserve(SrcNodes, DstNodes, Edges); }
	TBNEGraph(const TBNEGraph& Graph) : MxSrcNId(Graph.MxSrcNId), MxDstNId(Graph.MxDstNId), MxEId(Graph.MxEId), SrcNodeH(Graph.SrcNodeH), DstNodeH(Graph.DstNodeH), EdgeH(Graph.EdgeH) {}
	TBNEGraph(TSIn& SIn) : MxSrcNId(SIn), MxDstNId(SIn), MxEId(SIn), SrcNodeH(SIn), DstNodeH(SIn), EdgeH(SIn) {}
	void Save(TSOut& SOut) const { MxSrcNId.Save(SOut); MxDstNId.Save(SOut); MxEId.Save(SOut); SrcNodeH.Save(SOut); DstNodeH.Save(SOut); EdgeH.Save(SOut); }
	static PBNEGraph New() { return PBNEGraph(new TBNEGraph()); }
	static PBNEGraph New(const int& SrcNodes, const int& DstNodes, const int& Edges) { return PBNEGraph(new TBNEGraph(SrcNodes, DstNodes, Edges)); }
	static PBNEGraph Load(TSIn& SIn) { return PBNEGraph(new TBNEGraph(SIn)); }
	bool HasFlag(const TGraphFlag& Flag) const;
	TBNEGraph& operator = (const TBNEGraph& Graph) { if (this != &Graph) { MxSrcNId=Graph.MxSrcNId; MxDstNId=Graph.MxDstNId; MxEId=Graph.MxEId; SrcNodeH=Graph.SrcNodeH; DstNodeH=Graph.DstNodeH; EdgeH=Graph.EdgeH; } return *this; }
	// nodes
	int GetSrcNodes() const { return SrcNodeH.Len(); }
	int AddSrcNode(int NId = -1);
	int AddSrcNode(const TNodeI& NodeId) { return AddSrcNode(NodeId.GetId()); }
	void DelSrcNode(const int& NId);
	void DelSrcNode(const TNode& NodeI) {DelSrcNode(NodeI.GetId()); }
	bool IsSrcNode(const int& NId) const { return SrcNodeH.IsKey(NId); }
	TNodeI BegSrcNI() const { return TNodeI(SrcNodeH.BegI(), this); }
	TNodeI EndSrcNI() const { return TNodeI(SrcNodeH.EndI(), this); }
	TNodeI GetSrcNI(const int& NId) const { return TNodeI(SrcNodeH.GetI(NId), this); }

	int GetDstNodes() const { return DstNodeH.Len(); }
	int AddDstNode(int NId = -1);
	int AddDstNode(const TNodeI& NodeId) { return AddDstNode(NodeId.GetId()); }
	void DelDstNode(const int& NId);
	void DelDstNode(const TNode& NodeI) { DelDstNode(NodeI.GetId()); }
	bool IsDstNode(const int& NId) const { return DstNodeH.IsKey(NId); }
	TNodeI BegDstNI() const { return TNodeI(DstNodeH.BegI(), this); }
	TNodeI EndDstNI() const { return TNodeI(DstNodeH.EndI(), this); }
	TNodeI GetDstNI(const int& NId) const { return TNodeI(DstNodeH.GetI(NId), this); }

	int GetNodes() const { return GetSrcNodes() + GetDstNodes(); }

	// edges
	int GetEdges() const { return EdgeH.Len(); }
	int AddEdge(const int& SrcNId, const int& DstNId, const int& Dat=-1, int EId=-1);
	int AddEdge(const TEdgeI& EdgeI) { return AddEdge(EdgeI.GetSrcNId(), EdgeI.GetDstNId(), EdgeI.GetDat(), EdgeI.GetId()); }
	void DelEdge(const int& EId);
	void DelEdge(const int& SrcNId, const int& DstNId); // deletes all edges between the two nodes
	bool IsEdge(const int& EId) const { return EdgeH.IsKey(EId); }
	bool IsEdge(const int& SrcNId, const int& DstNId) const { int EId; return IsEdge(SrcNId, DstNId, EId); }
	...
	int GetRndSrcNId(TRnd& Rnd=TInt::Rnd) { return SrcNodeH.GetKey(SrcNodeH.GetRndKeyId(Rnd, 0.8)); }
	TNodeI GetRndSrcNI(TRnd& Rnd=TInt::Rnd) { return GetSrcNI(GetRndSrcNId(Rnd)); }
	int GetRndDstNId(TRnd& Rnd=TInt::Rnd) { return DstNodeH.GetKey(DstNodeH.GetRndKeyId(Rnd, 0.8)); }
	TNodeI GetRndDstNI(TRnd& Rnd=TInt::Rnd) { return GetDstNI(GetRndDstNId(Rnd)); }
	int GetRndEId(TRnd& Rnd=TInt::Rnd) { return EdgeH.GetKey(EdgeH.GetRndKeyId(Rnd, 0.8)); }
	TEdgeI GetRndEI(TRnd& Rnd=TInt::Rnd) { return GetEI(GetRndEId(Rnd)); }
	void GetSrcNIdV(TIntV& NIdV) const;
	void GetDstNIdV(TIntV& NIdV) const;
	void GetEIdV(TIntV& EIdV) const;

	bool Empty() const { return GetNodes()==0; }
	void Clr() { MxSrcNId=0; MxDstNId=0; MxEId=0; SrcNodeH.Clr(); DstNodeH.Clr(); EdgeH.Clr(); }
	void Reserve(const int& SrcNodes, const int& DstNodes, const int& Edges) {
		if(SrcNodes>0) SrcNodeH.Gen(SrcNodes/2);
		if(DstNodes>0) DstNodeH.Gen(DstNodes/2);
		if(Edges>0) EdgeH.Gen(Edges/2); }
	void Defrag(const bool& OnlyNodeLinks=false);
	bool IsOk(const bool& ThrowExcept=true) const;
	void Dump(FILE *OutF=stdout) const;

	static PBNEGraph GetSmallGraph();
	friend class TPt<TBNEGraph>;
};
