...
public:
   ...
   public:
    ...
    explicit TUNGraph(const int& Nodes, const int& Edges)
        : MxNId(0) {
        Reserve(Nodes, Edges);
    }
    ...
    int AddNode(const int& NId, const TVecPool<TInt>& Pool,
                const int& NIdVId);
...
public:
    ...
    int AddNode(const int& NId, const TIntV& InNIdV,
                const TIntV& OutNIdV);
    int AddNode(const int& NId, const TVecPool<TInt>& Pool,
                const int& SrcVId, const int& DstVId);
    void DelNode(const int& NId);
    void DelNode(const TNode& NodeI) { DelNode(NodeI.GetId()); }
    bool IsNode(const int& NId) const { return NodeH.IsKey(NId); }
    TNodeI BegNI() const { return TNodeI(NodeH.BegI()); }
    ...
    const TNode& GetNodeC(const int& NId) const {
        return NodeH.GetDat(NId);
    }
    // edges
    int GetEdges() const;
...
public:
    ...
    class TEdge {
       ...
       public:
        ...
        TEdge(const int& EId, const int& SourceNId,
              const int& DestNId)
    ...
    class TNodeI {
       ...
       public:
        ...
        int GetInNId(const int& EdgeN) const {
            return Graph->GetEdge(NodeHI.GetDat().GetInEId(EdgeN))
                .GetSrcNId();
        }
        int GetOutNId(const int& EdgeN) const {
            return Graph->GetEdge(NodeHI.GetDat().GetOutEId(EdgeN))
                .GetDstNId();
        }
        int GetNbhNId(const int& EdgeN) const {
            const TEdge& E =
                Graph->GetEdge(NodeHI.GetDat().GetNbhEId(EdgeN));
            return GetId() == E.GetSrcNId() ? E.GetDstNId()
                                            : E.GetSrcNId();
        }
        bool IsInNId(const int& NId) const;
        bool IsOutNId(const int& NId) const;
...
class TBNEGraph {
   ...
   public:
    class TNode {
       ...
       public:
        ...
        bool IsEId(const int& EId) const {
    ...
    class TEdge {
       ...
       public:
        ...
        TEdge(const int& EId, const int& SourceNId,
              const int& DestNId, const int& Data)
            : Id(EId),
              SrcNId(SourceNId),
              DstNId(DestNId),
              Dat(Data) {}
        TEdge(const TEdge& Edge)
            : Id(Edge.Id),
              ...
              Dat(Edge.Dat) {}
        TEdge(TSIn& SIn)
            : Id(SIn), SrcNId(SIn), DstNId(SIn), Dat(SIn) {}
        void Save(TSOut& SOut) const {
            Id.Save(SOut);
            SrcNId.Save(SOut);
            DstNId.Save(SOut);
   ...
   public:
    ...
    static PBNEGraph Load(TSIn& SIn) {
        return PBNEGraph(new TBNEGraph(SIn));
    }
    bool HasFlag(const TGraphFlag& Flag) const;
    ...
    int AddSrcNode(const TNodeI& NodeId) {
        return AddSrcNode(NodeId.GetId());
    }
    void DelSrcNode(const int& NId);
    void DelSrcNode(const TNode& NodeI) {
        DelSrcNode(NodeI.GetId());
    }
    bool IsSrcNode(const int& NId) const {
        return SrcNodeH.IsKey(NId);
    }
    ...
    int AddDstNode(const TNodeI& NodeId) {
        return AddDstNode(NodeId.GetId());
    }
    void DelDstNode(const int& NId);
    void DelDstNode(const TNode& NodeI) {
        DelDstNode(NodeI.GetId());
    }
    bool IsDstNode(const int& NId) const {
        return DstNodeH.IsKey(NId);
    }
    ...
    int GetEdges() const { return EdgeH.Len(); }
    int AddEdge(const int& SrcNId, const int& DstNId,
                const int& Dat = -1, int EId = -1);
    int AddEdge(const TEdgeI& EdgeI) {
        return AddEdge(EdgeI.GetSrcNId(), EdgeI.GetDstNId(),
                       EdgeI.GetDat(), EdgeI.GetId());
    }
    void DelEdge(const int& EId);
    ...
    void Reserve(const int& SrcNodes, const int& DstNodes,
                 const int& Edges) {
