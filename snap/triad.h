...
template<class PGraph> double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV, int SampleNodes) {
	...
	for (int d = 0; d < DegSumCnt.Len(); d++) DegToCCfV.Add(TFltPr(DegSumCnt.GetKey(d).Val, DegSumCnt[d].Val1() / DegSumCnt[d].Val2()));
	DegToCCfV.Sort();
	return SumCcf / double(NIdCOTriadV.Len());
}
...
double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV, int& ClosedTriads,
	...
	for (int i = 0; i < NIdCOTriadV.Len(); i++) {
		...
		TFltPr& SumCnt = DegSumCnt.AddDat(
				Graph->GetNI(NIdCOTriadV[i].Val1).GetDeg());
		SumCnt.Val1 += Ccf;
		SumCnt.Val2 += 1;
		SumCcf += Ccf;
	}
...
inline int GetCmnNbhs<PUNGraph> (const PUNGraph& Graph, const int& NId1,
	...
	while (i < NI1.GetDeg() && j < NI2.GetDeg()) {
		...
		if (j < NI2.GetDeg() && nid == NI2.GetNbhNId(j) && nid != NId1 && nid
				!= NId2) {
...
double TNetConstraint<PGraph>::GetNodeC(const int& NId) const {
	...
	for (int i = KeyId + 1; i < NodePrCH.Len() && NodePrCH.GetKey(i).Val1()
			== NId; i++) {
		Constraint += NodePrCH[i];
	}
	return Constraint;
