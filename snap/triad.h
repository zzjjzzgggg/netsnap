...
template <class PGraph>
double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV,
                  int SampleNodes = -1);
template <class PGraph>
double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV, int& ClosedTriads,
                  int& OpenTriads, int SampleNodes = -1);
...
// get triples of connected nodes (regardless of the number of edges between
// each pair of nodes)
template <class PGraph>
int GetTriads(const PGraph& Graph, int SampleNodes = -1);
template <class PGraph>
int GetTriads(const PGraph& Graph, int& ClosedTriads, int& OpenTriads,
              int SampleNodes);
template <class PGraph>
...
template <class PGraph>
int GetNodeTriads(const PGraph& Graph, const int& NId, int& ClosedTriads,
                  int& OpenTriads);
template <class PGraph>
...
template <class PGraph>
double GetClustCf(const PGraph& Graph, int SampleNodes) {
    TIntTrV NIdCOTriadV;
    ...
    for (int i = 0; i < NIdCOTriadV.Len(); i++) {
        ...
        if (OpenCnt > 0) SumCcf += NIdCOTriadV[i].Val2() / OpenCnt;
    }
    IAssert(SumCcf >= 0);
...
template <class PGraph>
double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV, int SampleNodes) {
    TIntTrV NIdCOTriadV;
    GetTriads(Graph, NIdCOTriadV, SampleNodes);
    ...
    for (int i = 0; i < NIdCOTriadV.Len(); i++) {
        ...
        TFltPr& SumCnt =
            DegSumCnt.AddDat(Graph->GetNI(NIdCOTriadV[i].Val1).GetDeg());
        SumCnt.Val1 += Ccf;
        SumCnt.Val2 += 1;
        SumCcf += Ccf;
    }
    ...
    for (int d = 0; d < DegSumCnt.Len(); d++)
        DegToCCfV.Add(TFltPr(DegSumCnt.GetKey(d).Val,
                             DegSumCnt[d].Val1() / DegSumCnt[d].Val2()));
    DegToCCfV.Sort();
    return SumCcf / double(NIdCOTriadV.Len());
}
...
template <class PGraph>
double GetClustCf(const PGraph& Graph, TFltPrV& DegToCCfV, int& ClosedTriads,
                  int& OpenTriads, int SampleNodes) {
    TIntTrV NIdCOTriadV;
    GetTriads(Graph, NIdCOTriadV, SampleNodes);
    THash<TInt, TFltPr> DegSumCnt;
    double SumCcf = 0.0;
...
int GetTriads(const PGraph& Graph, int& ClosedTriads, int& OpenTriads,
    ...
    TIntTrV NIdCOTriadV;
    GetTriads(Graph, NIdCOTriadV, SampleNodes);
    uint64 closedTriads = 0, openTriads = 0;
    for (int i = 0; i < NIdCOTriadV.Len(); i++) {
        closedTriads += NIdCOTriadV[i].Val2;
        openTriads += NIdCOTriadV[i].Val3;
    }
...
/// Note: pretends that the graph is undirected (count unique connected triples
/// of nodes)
template <class PGraph>
void GetTriads(const PGraph& Graph, TIntTrV& NIdCOTriadV, int SampleNodes) {
    ...
    if (SampleNodes == -1) SampleNodes = Graph->GetNodes();
    ...
    for (int node = 0; node < SampleNodes; node++) {
        ...
        for (int e = 0; e < NI.GetOutDeg(); e++) {
            if (NI.GetOutNId(e) != NI.GetId()) NbhH.AddKey(NI.GetOutNId(e));
        }
        if (IsDir) {
            for (int e = 0; e < NI.GetInDeg(); e++) {
                if (NI.GetInNId(e) != NI.GetId()) NbhH.AddKey(NI.GetInNId(e));
            }
        }
        // count connected neighbors
        int OpenCnt = 0, CloseCnt = 0;
        for (int srcNbh = 0; srcNbh < NbhH.Len(); srcNbh++) {
            const typename PGraph::TObj::TNodeI SrcNode =
                Graph->GetNI(NbhH.GetKey(srcNbh));
            for (int dstNbh = srcNbh + 1; dstNbh < NbhH.Len(); dstNbh++) {
                const int dstNId = NbhH.GetKey(dstNbh);
                if (SrcNode.IsNbhNId(dstNId))
                    CloseCnt++;  // is edge
                else
                    OpenCnt++;
            }
...
int GetNodeTriads(const PGraph& Graph, const int& NId, int& ClosedTriads,
    ...
    for (int e = 0; e < NI.GetOutDeg(); e++) {
        // exclude self edges
        if (NI.GetOutNId(e) != NI.GetId()) NbhSet.AddKey(NI.GetOutNId(e));
    }
    if (Graph->HasFlag(gfDirected)) {
        for (int e = 0; e < NI.GetInDeg(); e++) {
            // exclude self edges
            if (NI.GetInNId(e) != NI.GetId()) NbhSet.AddKey(NI.GetInNId(e));
        }
    }
    // count connected neighbors
    for (int srcNbh = 0; srcNbh < NbhSet.Len(); srcNbh++) {
        const typename PGraph::TObj::TNodeI SrcNode =
            Graph->GetNI(NbhSet.GetKey(srcNbh));
...
int GetNodeTriadsAll(const PGraph& Graph, const int& NId, int& ClosedTriads) {
    ...
    ClosedTriads = 0;
    if (NI.GetDeg() < 2) return 0;
    TIntH NbhWt(NI.GetDeg());  // find neighborhood
    int nid;
    for (int e = 0; e < NI.GetOutDeg(); e++) {
        nid = NI.GetOutNId(e);
        if (nid != NId) NbhWt(nid) += 1;  // exclude self edges
    }
    if (Graph->HasFlag(gfDirected)) {
        for (int e = 0; e < NI.GetInDeg(); e++) {
            nid = NI.GetInNId(e);
    ...
    for (int i = 0; i < NbhWt.Len(); i++) {
        ...
        for (int j = i + 1; j < NbhWt.Len(); j++) {
            ...
            if (DstNI.GetDeg() < SrcNI.GetDeg()) {
                // only check the small degree node
                tmpNI = DstNI;
                tarNId = srcNId;
            } else {
                tmpNI = SrcNI;
...
int GetCmnNbhs(const PGraph& Graph, const int& NId1, const int& NId2,
    ...
    for (int i = 0; i < NI1.GetDeg(); i++) {
        ...
        if (nid != NId1 && nid != NId2) NSet1.AddKey(nid);
    }
    for (int i = 0; i < NI2.GetDeg(); i++) {
        const int nid = NI2.GetNbhNId(i);
        if (NSet1.IsKey(nid)) NSet2.AddKey(nid);
    }
    NSet2.GetKeyV(NbhV);
    return NbhV.Len();
}
...
template <>
inline int GetCmnNbhs<PUNGraph>(const PUNGraph& Graph, const int& NId1,
                                const int& NId2, TIntV& NbhV) {
    if (!Graph->IsNode(NId1) || !Graph->IsNode(NId2)) {
        NbhV.Clr(false);
        return 0;
    }
    ...
    while (i < NI1.GetDeg() && j < NI2.GetDeg()) {
        const int nid = NI1.GetNbhNId(i);
        while (j < NI2.GetDeg() && NI2.GetNbhNId(j) < nid) {
            j++;
        }
        if (j < NI2.GetDeg() && nid == NI2.GetNbhNId(j) && nid != NId1 &&
            nid != NId2) {
...
int GetLen2Paths(const PGraph& Graph, const int& NId1, const int& NId2,
    ...
    for (int e = 0; e < NI.GetOutDeg(); e++) {
        const typename PGraph::TNodeI MidNI = Graph->GetNI(NI.GetOutNId(e));
        if (MidNI.IsOutNId(NId2)) NbhV.Add(MidNI.GetId());
    }
    return NbhV.Len();
}
...
public:
    PGraph Graph;
    THash<TIntPr, TFlt>
        NodePrCH;  // pairs of nodes that have non-zero network constraint
public:
...
TNetConstraint<PGraph>::TNetConstraint(const PGraph& GraphPt,
                                       const bool& CalcaAll)
    : Graph(GraphPt) {
    CAssert(!HasGraphFlag(typename PGraph::TObj,
                          gfMultiGraph));  // must not be multigraph
    if (CalcaAll) {
        CalcConstraints();
    }
}
...
double TNetConstraint<PGraph>::GetNodeC(const int& NId) const {
    ...
    for (int i = KeyId + 1;
         i < NodePrCH.Len() && NodePrCH.GetKey(i).Val1() == NId; i++) {
        Constraint += NodePrCH[i];
    }
    return Constraint;
}
...
void TNetConstraint<PGraph>::CalcConstraints() {
    // add edges
    for (typename PGraph::TObj::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI();
         EI++) {
...
void TNetConstraint<PGraph>::CalcConstraints(const int& NId) {
    ...
    for (int e = 0; e < StartNI.GetOutDeg(); e++) {
        typename PGraph::TObj::TNodeI MidNI =
            Graph->GetNI(StartNI.GetOutNId(e));
...
void TNetConstraint<PGraph>::Dump() const {
    ...
    for (int e = 0; e < NodePrCH.Len(); e++) {
        printf("  %4d %4d  :  %f\n", NodePrCH.GetKey(e).Val1(),
               NodePrCH.GetKey(e).Val2(), NodePrCH[e].Val);
    }
    printf("\n");
}
