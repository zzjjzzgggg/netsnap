...
/**
 * Return an integer vector, where the CCDF[i] represents the number of
 * population >= i. So when we should shift it to left one position when return.
 */
void TGUtil::GetCCdf(const TIntPrV& PdfV, TIntPrV& CCdfV) {
  CCdfV = PdfV;
  for (int i = CCdfV.Len() - 2; i >= 0; i--) CCdfV[i].Val2 += CCdfV[i + 1].Val2;
}
...
void TGUtil::GetCCdf(const TIntFltKdV& PdfV, TIntFltKdV& CCdfV) {
  CCdfV = PdfV;
  for (int i = CCdfV.Len() - 2; i >= 0; i--) CCdfV[i].Dat += CCdfV[i + 1].Dat;
}
...
void TGUtil::MakeExpBins(const TFltPrV& XYValV, TFltPrV& ExpXYValV,
                         const double& BinFactor, const double& MinYVal) {
  TGnuPlot::MakeExpBins(XYValV, ExpXYValV, BinFactor, MinYVal);
}

void TGUtil::MakeExpBins(const TFltKdV& XYValV, TFltKdV& ExpXYValV,
                         const double& BinFactor, const double& MinYVal) {
  TGnuPlot::MakeExpBins(XYValV, ExpXYValV, BinFactor, MinYVal);
}

void TGUtil::MakeExpBins(const TFltV& YValV, TFltV& ExpYValV,
...
void TGUtil::MakeExpBins(const TIntV& YValV, TIntV& ExpYValV,
                         const double& BinFactor) {
  ExpYValV.Clr(true);
  int prevI = 0;
  for (int i = 0; i < YValV.Len();) {
...
bool TStrUtil::GetXmlTagNmVal2(TXmlLx& XmlLx, TChA& TagNm, TChA& TagVal,
...
TChA TStrUtil::GetDomNm2(const TChA& UrlChA) {
  ...
  if (Dom.IsPrefix("www.")) {
    return Dom.GetSubStr(4, TInt::Mx);
  } else {
    return Dom;
  }
}
...
TChA TStrUtil::GetWebsiteNm(const TChA& PostUrlStr) {
  ...
  if (DomNm == "blogs.msdn.com" || DomNm == "ameblo.jp" ||
      DomNm == "xfruits.com" || DomNm == "scienceblogs.com" ||
      DomNm == "blogs.sun.com" || DomNm == "blog.wired.com" ||
      DomNm == "weblogs.asp.net" || DomNm == "blogs.technet.com" ||
      DomNm == "blogs.guardian.co" || DomNm == "blogs.clarin.com" ||
      DomNm == "blogs.sun.com" || DomNm == "blog.wired.com" ||
      DomNm == "weblogs.asp.net" || DomNm == "blogs.technet.com" ||
      DomNm == "blogs.guardian.com" || DomNm == "blogs.clarin.com" ||
      DomNm == "blogs.zdnet.com" || DomNm == "blogs.citypages.com" ||
      DomNm == "voices.washingtonpost.com" || DomNm == "blog.tv2.dk" ||
      DomNm == "blogs.menomoneefallsnow.com" ||
      DomNm == "weblogs.baltimoresun.com" || DomNm == "eonline.com") {
  ...
  if (DomNm == "digg.com") {
    if (PostUrlStr.IsPrefix("http://digg.com/submit?")) {
      ...
      if (Url != -1) {
        return GetWebsiteNm(
            PostUrlStr.GetSubStr(Url + 5, PostUrlStr.SearchCh('&', Url + 5)));
      }
    } else {
      return PostUrlStr.GetSubStr(7, GetNthOccurence(PostUrlStr, 4) - 1);
    }
  ...
  // http://nydailynews.com/blogs/subwaysquawkers/2008/08/anaheim-is-no-magic-kingdom-fo.html
  if (PostUrlStr.IsPrefix("http://nydailynews.com/blogs/") ||
      PostUrlStr.IsPrefix("http://bbc.co.uk/blogs/") ||
      PostUrlStr.IsPrefix("http://nydailynews.com/blogs/") ||
      PostUrlStr.IsPrefix("http://newsbusters.org/blogs/")) {
  ...
  if (DomNm == "news.google.com") {  // redirect
    ...
    if (UrlPos != -1) {
      return GetWebsiteNm(PostUrlStr.GetSubStr(
          UrlPos + 5, PostUrlStr.SearchCh('&', UrlPos + 5)));
    }
  }
  ...
  if (DomNm == "bloggrevyen.no") {  // redirect
    ...
    if (Http2 != -1) {
      return GetWebsiteNm(
          PostUrlStr.GetSubStr(Http2 + 1, PostUrlStr.Len() - 1));
    }
  }
  ...
  if (DomNm.IsSuffix(".rd.yahoo.com")) {
    ...
    if (Http2 != -1) {
      return GetWebsiteNm(
          PostUrlStr.GetSubStr(Http2 + 9, PostUrlStr.Len() - 1));
    }
  }
  return DomNm;
}
...
bool TStrUtil::GetNormalizedUrl(const TChA& UrlIn, const TChA& BaseUrl,
                                TChA& UrlOut) {
  UrlOut = UrlIn;
  ...
  if (!(UrlOut.IsPrefix("http://") || UrlOut.IsPrefix("ftp://"))) {
    ...
    if (UrlIn.Empty() ||
        !(BaseUrl.IsPrefix("http://") || BaseUrl.IsPrefix("ftp://"))) {
...
bool TStrUtil::StripEnd(const TChA& Str, const TChA& SearchStr, TChA& NewStr) {
  ...
  for (int i = 0; i < SearchStrLen; i++) {
    if (Str[StrLen - i - 1] != SearchStr[SearchStrLen - i - 1]) {
      return false;
    }
  }
...
// space seprated sequence of words (includes all non-blank characters, i.e.,
// punctuations)
...
int TStrUtil::SplitWords(TChA& ChA, TVec<char*>& WrdV, const bool& SplitOnWs) {
  ...
  for (char* c = (char*)ChA.CStr(); *c; c++) {
    if ((SplitOnWs && *c == ' ') || (!SplitOnWs && !TCh::IsAlNum(*c))) {
      *c = 0;
      if (!WrdV.Empty() && strlen(WrdV.Last()) == 0) {
        WrdV.DelLast();
      }
      WrdV.Add(c + 1);
    }
...
int TStrUtil::SplitOnCh(TChA& ChA, TVec<char*>& WrdV, const char& Ch,
  ...
  for (char* c = (char*)ChA.CStr(); *c; c++) {
    if (*c == Ch) {
      *c = 0;
      if (SkipEmpty && !WrdV.Empty() && strlen(WrdV.Last()) == 0) {
        WrdV.DelLast();
      }
      WrdV.Add(c + 1);
    }
...
int TStrUtil::SplitSentences(TChA& ChA, TVec<char*>& SentenceV) {
  ...
  for (; c < E; c++) {
    if (c < E && (*c == '.' || *c == '!' || *c == '?') &&
        !TCh::IsAlNum(*(c + 1))) {  // end of sentence
      if (c < E && *(c + 1) == '"') {
        *c = '"';
        c++;
      }  // blah." --> blah"
      ...
      *c = 0;
      c++;
      char* e = c - 1;
      while (e > B && *e != '"' && !TCh::IsAlNum(*e)) {
        *e = 0;
        e--;
      }  // skip trailing non-alpha-num chars
      while (c < E &&
             !(TCh::IsAlNum(*c) || (*c == '"' && TCh::IsAlNum(*(c + 1))))) {
        c++;
      }  // sentence starts with AlNum or "AlNum
      if (c < E) {
        SentenceV.Add(c);
      }
    }
...
void TStrUtil::GetWIdV(const TStrHash<TInt>& StrH, const char* CStr,
                       TIntV& WIdV) {
  const int NotWId = -1;
  TChA ChA(CStr);
  TVec<char*> WrdV;
...
bool TStrUtil::GetTmFromStr(const char* TmStr, TSecTm& Tm) {
  ...
  int Day = atoi(WrdV[d]);
  if (Mon <= 0) {
    Mon = MonthV1.SearchForw(WrdV[m]) + 1;
  }
  if (Mon <= 0) {
    Mon = MonthV2.SearchForw(WrdV[m]) + 1;
  }
  if (Mon == 0) {
    return false;
  }
...
TStr TStrUtil::GetStdName(TStr AuthorName) {
  ...
  int i, pos = 0;
  while (pos < AuthorName.Len() &&
         (AuthorName[pos] != '#' && !TCh::IsNum(AuthorName[pos]))) {
    pos++;
  }
  ...
  if (AuthorName.SearchStr("figures") != -1 ||
      AuthorName.SearchStr("macros") != -1 ||
      AuthorName.SearchStr("univ") != -1 ||
      AuthorName.SearchStr("institute") != -1) {
    return TStr::GetNullStr();
  }
