...
private:
	...
	double GetCount(const TAnfBitV& BitV, const uint64& NIdOffset) const {return pow(2.0, AvgLstZero(BitV, NIdOffset))/0.77351;}
public:
	...
	void GetNodeAnf(const int& SrcNId, TIntFltKdV& DistNbhsV, const int& MxDist=-1, const bool& IsDir=false);
	void GetGraphAnf(TIntFltKdV& DistNbhsV, const int& MxDist=-1, const bool& IsDir=false);
};
...
void TGraphAnf<PGraph>::InitAnfBits(TAnfBitV& BitV) {
	...
	for (typename PGraph::TObj::TNodeI NI=Graph->BegNI(); NI!=Graph->EndNI(); NI++) {
		...
		for (int approx=0; approx<NApprox; approx++) {
			...
			for (SetBit = 0; (RndNum & (1<<SetBit))==0 && SetBit<NBits; SetBit++); // find the position of the right least 1-bit in RndNum;
			if (SetBit >= NBits) SetBit = NBits-1;
			const int BitPos = ApproxBytes*SetBit + approx/8; // ApproxBytes= 4, hence, BitPos = SetBit<<2 + approx/8. !!! very strange organization, but it is correct
...
template <class PGraph>
double TGraphAnf<PGraph>::AvgLstZero(const TAnfBitV& BitV, const uint64& NIdOffset) const {
	int approx, bit, AvgBitPos = 0;
	uchar* BitVPt = (uchar*)BitV.BegI();
	for (approx=0; approx<NApprox; approx++) {
		for (bit = 0; bit<NBits && (*(BitVPt+NIdOffset+ApproxBytes*bit+approx/8) & (1<<(approx%8))); bit++);  // found zero
...
void TGraphAnf<PGraph>::GetGraphAnf(TIntFltKdV& DistNbhsV, const int& MxDist, const bool& IsDir) {
	...
	for (int dist=2; dist<=(MxDist==-1?TInt::Mx:MxDist); dist++) {
		...
		NPairs = 0.0;
		for (v = NIdToBitPosH.FFirstKeyId(); NIdToBitPosH.FNextKeyId(v);) NPairs += GetCount(CurBitsV, NIdToBitPosH[v]);
...
template <class PGraph> void GetAnf(const PGraph& Graph, const int& SrcNId, TIntFltKdV& DistNbhsV, const int& MxDist=-1, const bool& IsDir=false, const int& NApprox=32); // number of reachable nodes at distance Dist
template <class PGraph> void GetAnf(const PGraph& Graph, TIntFltKdV& DistNbhsV, const int& MxDist=-1, const bool& IsDir=false, const int& NApprox=32); // number of pairs of reachable nodes at distance Dist
...
double GetAnfEffDiam(const PGraph& Graph, const int NRuns, int NApprox) {
	...
	for (int r = 0; r < NRuns; r++) Mom.Add(TSnap::GetAnfEffDiam(Graph, IsDir, 0.9, NApprox));
	Mom.Def();
	return Mom.GetMean();
}
