...
void PlotEigValRank(const PUNGraph& Graph, const int& EigVals,
                    const TStr& FNmPref, TStr DescStr) {
  TFltV EigValV;
  ...
  TGnuPlot::PlotValV(
      EigValV, "eigVal." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Largest eig val = %f", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges(), EigValV[0].Val),
      "Rank", "Eigen value", gpsLog10XY, false, gpwLinesPoints);
}
...
void PlotEigValDistr(const PUNGraph& Graph, const int& EigVals,
  ...
  TGnuPlot::PlotValV(
      EigCntV, "eigDistr." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Largest eig val = %f", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges(), EigValV.Last().Val),
      "Eigen value", "Count", gpsAuto, false, gpwLinesPoints);
}

// Inverse participation ratio: normalize EigVec to have L2=1 and then I=sum_k
// EigVec[i]^4
// see Spectra of "real-world" graphs: Beyond the semicircle law by Farkas,
// Derenyi, Barabasi and Vicsek
void PlotInvParticipRat(const PUNGraph& Graph, const int& MaxEigVecs,
  ...
  if (EigIprV.Empty()) {
    DescStr += ". FAIL";
    EigIprV.Add(TFltPr(-1, -1));
    return;
  }
  TGnuPlot::PlotValV(
      EigIprV, "eigIPR." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Largest eig val = %f (%d values)",
                DescStr.CStr(), Graph->GetNodes(), Graph->GetEdges(),
                EigIprV.Last().Val1(), EigIprV.Len()),
      "Eigenvalue", "Inverse Participation Ratio of corresponding Eigenvector",
      gpsLog10Y, false, gpwPoints);
}
...
void PlotSngValRank(const PNGraph& Graph, const int& SngVals,
  ...
  TGnuPlot::PlotValV(
      SngValV, "sngVal." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Largest eig val = %f", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges(), SngValV[0].Val),
      "Rank", "Singular value", gpsLog10XY, false, gpwLinesPoints);
}
...
void PlotSngValDistr(const PNGraph& Graph, const int& SngVals,
  ...
  TGnuPlot::PlotValV(
      EigCntV, "sngDistr." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Largest eig val = %f", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges(), SngValV.Last().Val),
      "Singular value", "Count", gpsAuto, false, gpwLinesPoints);
}
...
void PlotSngVec(const PNGraph& Graph, const TStr& FNmPref, TStr DescStr) {
  ...
  TGnuPlot::PlotValV(
      BinV, "sngVecL." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Left signular vector", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges()),
      "Rank", "Component of left singular vector", gpsLog10XY, false,
      gpwLinesPoints);
  TGnuPlot::PlotValV(
      BinV, "sngVecL." + FNmPref,
      TStr::Fmt("%s. G(%d, %d). Right signular vector", DescStr.CStr(),
                Graph->GetNodes(), Graph->GetEdges()),
      "Rank", "Component of right singular vector", gpsLog10XY, false,
      gpwLinesPoints);
}
