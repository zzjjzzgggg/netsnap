...
public:
	...
	void PlotAll(const TStr& OutFNm, const TStr& Desc,
			const bool& DivByM = true);
	static double NonZ(double C) {
		return C == 0 ? 1.0 : C;
	}
	...
	void SampleCascade(const PGraph& InfCasc, const PGraph& NetCasc,
			const TIntH& NIdInfTmH, const double& PStep = 0.05,
			const int& NRuns = 1, const bool& DivByM = true) {
		for (int Run = 0; Run < NRuns; Run++) {
			for (double P = PStep; P <= 1.01; P += PStep) {
				TIntV NIdV;
				for (typename PGraph::TObj::TNodeI NI = InfCasc->BegNI(); NI < InfCasc->EndNI(); NI++) {
					if (TInt::Rnd.GetUniDev() < P) {
						NIdV.Add(NI.GetId());
					}
				}
	...
	template<class PGraph>
	void TakeStat(const PGraph& InfG, const PGraph& NetG,
			const TIntH& NIdInfTmH, const double& P, const bool& DivByM = true) {
		const double M = DivByM ? InfG->GetNodes() : 1;
		IAssert(M>=1);
		PGraph CcInf, CcNet; // largest connected component
		// connected components and sizes
		{
			TCnComV CnComV;
			TSnap::GetWccs(InfG, CnComV);
		...
		{
			int i1 = 0, i2 = 0, l1 = 0, l2 = 0, r1 = 0, r2 = 0, ENet = 0, EInf =
					0;
			double ci1 = 0, ci2 = 0, co1 = 0, co2 = 0;
			for (typename PGraph::TObj::TNodeI NI = InfG->BegNI(); NI
					< InfG->EndNI(); NI++) {
				if (NI.GetOutDeg() == 0 && NI.GetInDeg() > 0) {
					l1++;
				}
			...
			for (typename PGraph::TObj::TNodeI NI = NetG->BegNI(); NI
					< NetG->EndNI(); NI++) {
				if (NI.GetOutDeg() == 0 && NI.GetInDeg() > 0) {
					l2++;
				}
			...
			if (ci1 > 0)
				InDegInf.AddDat(P).Add(EInf / ci1);
			if (ci2 > 0)
				InDegNet.AddDat(P).Add(ENet / ci2);
			if (co1 > 0)
				OutDegInf.AddDat(P).Add(EInf / co1);
			if (co2 > 0)
				OutDegNet.AddDat(P).Add(ENet / co2);
			NLfInf.AddDat(P).Add(l1 / M);
			NLfNet.AddDat(P).Add(l2 / M);
			NRtInf.AddDat(P).Add(r1 / M);
			NRtNet.AddDat(P).Add(r2 / M);
			NIsoInf.AddDat(P).Add(i1 / M);
			NIsoNet.AddDat(P).Add(i2 / M);
		}
		// cascade depth
		{
			const double M1 = DivByM ? CcNet->GetNodes() : 1;
			IAssert(M1>=1);
			...
			for (int i = 0; i < HopCntV.Len(); i++) {
				if (MxN < HopCntV[i].Val2) {
					MxN = HopCntV[i].Val2;
					Lev = HopCntV[i].Val1;
				}
				if (i > 0 && HopCntV[i - 1].Val2 <= HopCntV[i].Val2) {
					IncL++;
				}
			}
			double D = 0;
			...
			if (c != 0 && D != 0) {
				D = D / c;
				DepthInf.AddDat(P).Add(D / M1);
				MxWidInf.AddDat(P).Add(MxN / M1);
				MxLevInf.AddDat(P).Add(Lev / D);
				IncLevInf.AddDat(P).Add(IncL / D);
			}
			...
			if (c != 0 && D != 0) {
				D = D / c;
				DepthNet.AddDat(P).Add(D / M1);
				MxWidNet.AddDat(P).Add(MxN / M1);
				MxLevNet.AddDat(P).Add(Lev / D);
				IncLevNet.AddDat(P).Add(IncL / D);
			}
		}
	}
};

PNGraph RunSICascade(PUNGraph G, const double& Beta, const int& CascSz,
		TIntH& NIdInfTmH);
PNGraph RunSICascade2(PUNGraph G, const double& Beta, const int& MxCascSz,
		TIntH& NIdInfTmH);
PNGraph AddSpuriousEdges(const PUNGraph& Graph, const PNGraph& Casc,
		TIntH NIdTmH);

int main(int argc, char* argv[]) {
	Env = TEnv(argc, argv, TNotify::StdNotify);
	Env.PrepArgs(
			TStr::Fmt("Cascades. build: %s, %s. Time: %s", __TIME__, __DATE__,
					TExeTm::GetCurTm()));
	TExeTm ExeTm;
	Try
		const TStr InFNm = Env.GetIfArgPrefixStr("-i:", "demo",
				"Input undirected graph");
		const TStr OutFNm = Env.GetIfArgPrefixStr("-o:", "demo",
				"Output file name prefix");
		const double Beta = Env.GetIfArgPrefixFlt("-b:", 0.1,
				"Beta (infection (i.e., cascade propagation) probability)");
		// load
		printf("Loading %s...", InFNm.CStr());
		PUNGraph Graph;
		if (InFNm == "demo") {
			Graph = TSnap::GenRndGnm<PUNGraph>(100, 200);
		} else {
		...
		CascStat.PlotAll(
				TStr::Fmt("%s-B%03d", OutFNm.CStr(), int(100 * Beta)),
				TStr::Fmt("%s N=%d  E=%d  Beta=%g", OutFNm.CStr(),
						Graph->GetNodes(), Graph->GetEdges(), Beta), DivByM);
...
void TCascadeStat::PlotAll(const TStr& OutFNm, const TStr& Desc, const bool& DivByM) {
	const TStr MStr = DivByM ? " / M (number of observed nodes)" : "";
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Number of connected components" + MStr);
		GP.AddPlot(NCascInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(NCascNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Size of largest connected component" + MStr);
		GP.AddPlot(MxSzInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(MxSzNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Average connected component size" + MStr);
		GP.AddPlot(AvgSzInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(AvgSzNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Number of isolated nodes" + MStr);
		GP.AddPlot(NIsoInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(NIsoNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Number of root nodes" + MStr);
		GP.AddPlot(NRtInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(NRtNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.AddPlot(NLfInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(NLfNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Average Out-Degree (of a non-leaf)");
		GP.AddPlot(OutDegInf, gpwLinesPoints, "Influence cascade", "lw 2",
				true, false, false, false, false, false);
		GP.AddPlot(OutDegNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Average In-Degree (of a non-root)");
		GP.AddPlot(InDegInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(InDegNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Average depth of largest component" + MStr);
		GP.AddPlot(DepthInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(DepthNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.AddPlot(MxWidInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(MxWidNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Level with maximum width / Depth");
		GP.AddPlot(MxLevInf, gpwLinesPoints, "Influence cascade", "lw 2", true,
				false, false, false, false, false);
		GP.AddPlot(MxLevNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
	{
		...
		GP.SetXYLabel("Fraction of observed data (P)",
				"Number of levels of increasing width / Depth");
		GP.AddPlot(IncLevInf, gpwLinesPoints, "Influence cascade", "lw 2",
				true, false, false, false, false, false);
		GP.AddPlot(IncLevNet, gpwLinesPoints, "Network cascade", "lw 2", true,
				false, false, false, false, false);
		GP.SavePng();
	}
}
...
PNGraph RunSICascade2(PUNGraph G, const double& Beta, const int& MxCascSz,	TIntH& NIdInfTmH) {
	...
	Q.Push(StartNId);
	while (!Q.Empty()) {
		const TUNGraph::TNodeI NI = G->GetNI(Q.Top());
		Q.Pop();
		for (int i = 0; i < NI.GetOutDeg(); i++) {
			if (TInt::Rnd.GetUniDev() < Beta && !NIdInfTmH.IsKey(
					NI.GetOutNId(i))) {
				Casc->AddNode(NI.GetOutNId(i));
				NIdInfTmH.AddDat(NI.GetOutNId(i), NIdInfTmH.Len());
				Casc->AddEdge(NI.GetId(), NI.GetOutNId(i));
				if (Casc->GetNodes() == MxCascSz) {
					return Casc;
				}
...
PNGraph AddSpuriousEdges(const PUNGraph& Graph, const PNGraph& Casc, TIntH NIdTmH) {
	...
	for (TNGraph::TNodeI NI = Casc->BegNI(); NI < Casc->EndNI(); NI++) {
		...
		for (int i = 0, j = 0; i < GNI.GetOutDeg(); i++) {
			...
			if (NIdTmH.IsKey(Dst) && Tm < NIdTmH.GetDat(Dst) && !NI.IsNbhNId(
					Dst)) {
