...
int main(int argc, char* argv[]) {
	...
	Try
		...
		if (InFNm.GetFExt().GetLc() == ".ungraph") {
			...
			TFIn FIn(InFNm);
			G = TSnap::ConvertGraph<PUNGraph>(TNGraph::Load(FIn), false);
		} else {
		...
		TGnuPlot::PlotValV(KCoreV,"kcore-" + OutFNm,
				TStr::Fmt("k-core decomposition of graph %s (%d, %d)",OutFNm.CStr(), G->GetNodes(), G->GetEdges()),
				"k (min node degree in the k-core)","Number of nodes in the k-core", gpsLog);
