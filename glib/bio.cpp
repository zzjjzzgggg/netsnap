...
void SaveIntFltKdV(const TIntFltKdV& list, const TStr& Fnm,
    ...
    for (int i = 0; i < list.Len(); i++)
        Fw->PutStrLn(
            TStr::Fmt("%d\t%.6e", list[i].Key.Val, list[i].Dat.Val));
}
...
void SaveIntH(const TIntH& hash, const TStr& Fnm, const TStr& anno) {
    ...
    for (int i = 0; i < hash.Len(); i++)
        Fw->PutStrLn(
            TStr::Fmt("%d\t%d", hash.GetKey(i).Val, hash[i].Val));
}
void SaveStrIntH(const TStrIntH& hash, const TStr& Fnm,
    ...
    for (int i = 0; i < hash.Len(); i++)
        Fw->PutStrLn(
            TStr::Fmt("%s\t%d", hash.GetKey(i).CStr(), hash[i].Val));
}
void SaveIntFltH(const TIntFltH& hash, const TStr& Fnm,
    ...
    for (int i = 0; i < hash.Len(); i++)
        Fw->PutStrLn(
            TStr::Fmt("%d\t%.6e", hash.GetKey(i).Val, hash[i].Val));
}
...
void SaveIntIntPrH(const TIntPrH& hash, const TStr& Fnm,
    ...
    for (int i = 0; i < hash.Len(); i++)
        Fw->PutStrLn(TStr::Fmt("%d\t%d\t%d", hash.GetKey(i).Val,
                               hash[i].Val1.Val, hash[i].Val2.Val));
}
...
void SaveIntPrSet(const TIntPrSet& hash, const TStr& Fnm,
    ...
    for (int i = 0; i < hash.Len(); i++) {
        TIntPr pr = hash.GetKey(i);
        Fw->PutStrLn(TStr::Fmt("%d\t%d", pr.Val1.Val, pr.Val2.Val));
    }
}
...
void LoadIntFltH(const TStr& fnm, TIntFltH& dat, const int c1,
                 const int c2) {
    TSsParser Ss(fnm);
    while (Ss.Next()) dat.AddDat(Ss.GetInt(c1), Ss.GetFlt(c2));
}
void LoadIntPrSet(const TStr& fnm, TIntPrSet& dat, const int c1,
                  const int c2) {
    TSsParser Ss(fnm);
    while (Ss.Next())
...
void LoadIntFltPrV(const TStr& fnm, TIntFltPrV& dat, const int c1,
                   const int c2) {
    TSsParser Ss(fnm);
    while (Ss.Next())
