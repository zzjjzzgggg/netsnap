...
void TRnd::SetSeed(const int& _Seed){
	Assert(_Seed>=0);
	if (_Seed==0) Seed=abs(int(TSysTm::GetPerfTimerTicks()));
	else Seed=_Seed;
}
...
double TRnd::GetNrmDev(const double& Mean, const double& SDev, const double& Mn, const double& Mx){
...
void TMem::Resize(const int& _MxBfL){
	if (_MxBfL<=MxBfL) return;
		...
		if (MxBfL*2<_MxBfL)MxBfL=_MxBfL;
		else MxBfL*=2;
	}
