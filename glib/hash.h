...
public:
  THashKeyDat() : Next(-1), HashCd(-1), Key(), Dat() {}
  THashKeyDat(const int& _Next, const int& _HashCd,
              const TKey& _Key)
...
public:
  static inline int GetPrimHashCd(const TKey& Key) {
    return Key.GetPrimHashCd();
  }
  static inline int GetSecHashCd(const TKey& Key) {
...
private:
  ...
  public:
    ...
    bool CmpKey, Asc;
    THashKeyDatCmp(THash<TKey, TDat, THashFunc>& _Hash,
                   const bool& _CmpKey, const bool& _Asc)
        : Hash(_Hash), CmpKey(_CmpKey), Asc(_Asc) {}
    bool operator()(const int& KeyId1,
                    const int& KeyId2) const {
      if (CmpKey) {
        if (Asc)
...
public:
  THash()
      : PortV(), KeyDatV(), AutoSizeP(true), FFreeKeyId(-1),
        FreeKeys(0) {}
  THash(const THash& Hash)
      : PortV(Hash.PortV), KeyDatV(Hash.KeyDatV),
        AutoSizeP(Hash.AutoSizeP),
        FFreeKeyId(Hash.FFreeKeyId), FreeKeys(Hash.FreeKeys) {
  }
  explicit THash(const int& ExpectVals,
                 const bool& _AutoSizeP = false);
  explicit THash(TSIn& SIn)
      : PortV(SIn), KeyDatV(SIn), AutoSizeP(SIn),
        FFreeKeyId(SIn), FreeKeys(SIn) {
    SIn.LoadCs();
  }
  void Load(TSIn& SIn) {
    PortV.Load(SIn);
  ...
  const TDat& operator()(const TKey& Key) const {
  ...
  void DelIfKey(const TKey& Key) {
    int KeyId;
    if (IsKey(Key, KeyId)) DelKeyId(KeyId);
  }
  ...
  void MarkDelKey(const TKey& Key);  // marks the record as
                                     // deleted - doesn't
                                     // delete Dat (to avoid
                                     // fragmentation)
  ...
  bool IsKeyId(const int& KeyId) const {
    return (0 <= KeyId) && (KeyId < KeyDatV.Len()) &&
  ...
  void GetKeyDat(const int& KeyId, TKey& Key,
                 TDat& Dat) const {
...
const unsigned int THash<
    TKey, TDat, THashFunc>::HashPrimeT[HashPrimes] = {
    3ul,         5ul,          11ul,         23ul,
...
template <class TKey, class TDat, class THashFunc>
uint THash<TKey, TDat, THashFunc>::GetNextPrime(
    const uint& Val) const {
  const uint *f = (const uint *)HashPrimeT, *m,
             *l = (const uint *)HashPrimeT + (int)HashPrimes;
...
void THash<TKey, TDat, THashFunc>::Resize() {
  ...
  for (int KeyId = 0; KeyId < KeyDatV.Len(); KeyId++) {
    ...
    if (KeyDat.HashCd != -1) {
      const int PortN = abs(
          THashFunc::GetPrimHashCd(KeyDat.Key) % PortV.Len());
...
THash<TKey, TDat, THashFunc>::THash(const int& ExpectVals,
    ...
    : PortV(GetNextPrime(ExpectVals / 2)),
      KeyDatV(ExpectVals, 0), AutoSizeP(_AutoSizeP),
      FFreeKeyId(-1), FreeKeys(0) {
  PortV.PutAll(TInt(-1));
}

template <class TKey, class TDat, class THashFunc>
bool THash<TKey, TDat, THashFunc>::operator==(
    const THash& Hash) const {
...
template <class TKey, class TDat, class THashFunc>
void THash<TKey, TDat, THashFunc>::Clr(const bool& DoDel,
                                       const int& NoDelLim,
                                       const bool& ResetDat) {
  if (DoDel) {
    PortV.Clr();
...
int THash<TKey, TDat, THashFunc>::AddKey(const TKey& Key) {
  if ((KeyDatV.Len() > 2 * PortV.Len()) || PortV.Empty())
    Resize();
  const int PortN =
      abs(THashFunc::GetPrimHashCd(Key) % PortV.Len());
  const int HashCd = abs(THashFunc::GetSecHashCd(Key));
  int PrevKeyId = -1, KeyId = PortV[PortN];
  while (KeyId != -1 &&
  ...
  if (KeyId == -1) {
    if (FFreeKeyId == -1)
      ...
      FreeKeys--;
      // KeyDatV[KeyId]=TKeyDat(-1, HashCd, Key); // slow
      // version
...
// return random KeyId even if the hash table contains deleted
// keys
// defrags the table if necessary
template <class TKey, class TDat, class THashFunc>
int THash<TKey, TDat, THashFunc>::GetRndKeyId(
    TRnd& Rnd, const double& EmptyFrac) {
  IAssert(!Empty());
...
void THash<TKey, TDat, THashFunc>::GetKeyV(
    TVec<TKey>& KeyV) const {
  KeyV.Gen(Len(), 0);
  int KeyId = FFirstKeyId();
  while (FNextKeyId(KeyId)) KeyV.Add(GetKey(KeyId));
}

template <class TKey, class TDat, class THashFunc>
void THash<TKey, TDat, THashFunc>::GetDatV(
    TVec<TDat>& DatV) const {
  DatV.Gen(Len(), 0);
  int KeyId = FFirstKeyId();
...
void THash<TKey, TDat, THashFunc>::GetKeyDatPrV(
    TVec<TPair<TKey, TDat>>& KeyDatPrV) const {
  KeyDatPrV.Gen(Len(), 0);
  TKey Key;
  TDat Dat;
  int KeyId = FFirstKeyId();
...
void THash<TKey, TDat, THashFunc>::GetDatKeyPrV(
    TVec<TPair<TDat, TKey>>& DatKeyPrV) const {
  DatKeyPrV.Gen(Len(), 0);
  TKey Key;
  TDat Dat;
  int KeyId = FFirstKeyId();
...
void THash<TKey, TDat, THashFunc>::Sort(const bool& CmpKey,
  ...
  IAssertR(IsKeyIdEqKeyN(),
           "THash::Sort only works when table has no deleted "
           "keys.");
  ...
  for (int i = 0; i < TargV.Len(); i++) MapV[TargV[i]] = i;
  for (int p = 0; p < PortV.Len(); p++) {
    if (PortV[p] != -1) PortV[p] = MapV[PortV[p]];
  }
...
typedef THash<TInt, TIntPr> TIntPrH;
typedef THash<TInt, TIntTr> TIntIntTrH;
...
/////////////////////////////////////////////////
// Big-String-Pool (holds up to 2 giga strings, storage
// overhead is 8(4) bytes per string)
// J: have to put it here since it uses TVec (can't be in
// dt.h)
...
/////////////////////////////////////////////////
// String-Hash-Table
template <class TDat, class TStringPool = TStrPool,
...
public:
  ...
  TStrHash(const PStringPool& StrPool)
      : PortV(), KeyDatV(), AutoSizeP(true), FFreeKeyId(-1),
        FreeKeys(0), Pool(StrPool) {}
  TStrHash(const int& Ports, const bool& _AutoSizeP = false,
           const PStringPool& StrPool = PStringPool())
      : PortV(Ports), KeyDatV(Ports, 0),
  ...
  // TDat& operator ()(const char *Key) { return AddDat(Key);
  // }
  // // add if not found
  ...
  void GetKeyDat(const int& KeyId, int& KeyO,
                 TDat& Dat) const {
    const TKeyDat& KeyDat = GetHashKeyDat(KeyId);
    KeyO = KeyDat.Key;
    Dat = KeyDat.Dat;
  }
  void GetKeyDat(const int& KeyId, const char*& Key,
                 TDat& Dat) const {
    const TKeyDat& KeyDat = GetHashKeyDat(KeyId);
    Key = KeyFromOfs(KeyDat.Key);
    Dat = KeyDat.Dat;
  }
  void GetKeyDat(const int& KeyId, TStr& Key,
                 TDat& Dat) const {
    const TKeyDat& KeyDat = GetHashKeyDat(KeyId);
    Key = KeyFromOfs(KeyDat.Key);
    Dat = KeyDat.Dat;
  }
  void GetKeyDat(const int& KeyId, TChA& Key,
                 TDat& Dat) const {
    const TKeyDat& KeyDat = GetHashKeyDat(KeyId);
    Key = KeyFromOfs(KeyDat.Key);
    Dat = KeyDat.Dat;
...
uint TStrHash<TDat, TStringPool, THashFunc>::GetNextPrime(
    const uint& Val) const {
  uint *f = (uint *)TIntH::HashPrimeT, *m,
       *l =
           (uint *)TIntH::HashPrimeT + (int)TIntH::HashPrimes;
...
void TStrHash<TDat, TStringPool, THashFunc>::Resize() {
  ...
  for (int i = 0; i < KeyDatV.Len(); i++) {
    ...
    if (KeyDat.HashCd != -1) {
      const int Port = abs(THashFunc::GetPrimHashCd(
                               Pool->GetCStr(KeyDat.Key)) %
                           NPorts);
...
TStrHash<TDat, TStringPool, THashFunc>::operator=(
    const TStrHash& Hash) {
  if (this != &Hash) {
    PortV = Hash.PortV;
    KeyDatV = Hash.KeyDatV;
...
int TStrHash<TDat, TStringPool, THashFunc>::AddKey(
  ...
  if ((AutoSizeP && KeyDatV.Len() > PortV.Len()) ||
      PortV.Empty())
    Resize();
  const int PortN =
      abs(THashFunc::GetPrimHashCd(Key) % PortV.Len());
  ...
  while (KeyId != -1 &&
         !(KeyDatV[KeyId].HashCd == HashCd &&
           Pool->Cmp(KeyDatV[KeyId].Key, Key) == 0)) {
    PrevKeyId = KeyId;
    KeyId = KeyDatV[KeyId].Next;
  }
  if (KeyId == -1) {
...
template <class TDat, class TStringPool, class THashFunc>
bool TStrHash<TDat, TStringPool, THashFunc>::FNextKeyId(
    int& KeyId) const {
  do
    KeyId++;
  while (KeyId < KeyDatV.Len() &&
...
template <class TDat, class TStringPool, class THashFunc>
void TStrHash<TDat, TStringPool, THashFunc>::GetKeyV(
    TVec<TStr>& KeyV) const {
  KeyV.Gen(Len(), 0);
  int KeyId = FFirstKeyId();
  while (FNextKeyId(KeyId)) KeyV.Add(GetKey(KeyId));
}
...
template <class TDat, class TStringPool, class THashFunc>
void TStrHash<TDat, TStringPool, THashFunc>::GetStrIdV(
    TIntV& StrIdV) const {
  StrIdV.Gen(Len(), 0);
  int KeyId = FFirstKeyId();
  while (FNextKeyId(KeyId)) StrIdV.Add(GetKeyOfs(KeyId));
}

template <class TDat, class TStringPool, class THashFunc>
void TStrHash<TDat, TStringPool, THashFunc>::GetDatV(
    TVec<TDat>& DatV) const {
  DatV.Gen(Len(), 0);
  int KeyId = FFirstKeyId();
  while (FNextKeyId(KeyId))
...
template <class TDat, class TStringPool, class THashFunc>
void TStrHash<TDat, TStringPool, THashFunc>::GetKeyDatPrV(
    TVec<TPair<TStr, TDat>>& KeyDatPrV) const {
  KeyDatPrV.Gen(Len(), 0);
  TStr Str;
  TDat Dat;
  int KeyId = FFirstKeyId();
...
template <class TDat, class TStringPool, class THashFunc>
void TStrHash<TDat, TStringPool, THashFunc>::GetDatKeyPrV(
    TVec<TPair<TDat, TStr>>& DatKeyPrV) const {
  DatKeyPrV.Gen(Len(), 0);
  TStr Str;
  TDat Dat;
  int KeyId = FFirstKeyId();
...
public:
  TCache() {}
  TCache(const TCache&);
  TCache(const int64& _MxMemUsed, const int& Ports,
         void* _RefToBs)
      : MxMemUsed(_MxMemUsed), CurMemUsed(0), KeyDatH(Ports),
        TimeKeyL(), RefToBs(_RefToBs) {}
...
template <class TKey, class TDat, class THashFunc>
void TCache<TKey, TDat, THashFunc>::Purge(
    const int64& MemToPurge) {
  while (!TimeKeyL.Empty() &&
         (MxMemUsed - CurMemUsed < MemToPurge)) {
...
void TCache<TKey, TDat, THashFunc>::Del(
    const TKey& Key, const bool& DoEventCall) {
  int KeyId = KeyDatH.GetKeyId(Key);
  if (KeyId != -1) {
    TKeyLNDatPr& KeyLNDatPr = KeyDatH[KeyId];
    TKeyLN KeyLN = KeyLNDatPr.Val1;
    TDat& Dat = KeyLNDatPr.Val2;
    if (DoEventCall) Dat->OnDelFromCache(Key, RefToBs);
...
class TStrHashF_DJB {
private:
  inline static unsigned int DJBHash(const char* Str,
                                     const ::TSize& Len) {
