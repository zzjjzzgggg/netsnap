...
public:
    TEnv()
        : ArgV(),
          ...
          Notify() {}
    TEnv(const TEnv&);
    TEnv(const int& _Args, char** _ArgV,
    ...
    void PrepArgs(const TStr& _HdStr = TStr(),
                  const int& _MnArgs = 1);
    ...
    TStr GetIfArgPrefixStr(const TStr& PrefixStr,
                           const TStr& DfVal = "",
                           const TStr& DNm = "") const;
    TStrV GetIfArgPrefixStrV(const TStr& PrefixStr,
                             TStrV& DfValV,
                             const TStr& DNm = "") const;
    TStrV GetIfArgPrefixStrV(const TStr& PrefixStr,
                             const TStr& DNm = "") const {
        TStrV DfValV;
        return GetIfArgPrefixStrV(PrefixStr, DfValV, DNm);
    }
    bool GetIfArgPrefixBool(const TStr& PrefixStr,
                            const bool& DfVal = false,
                            const TStr& DNm = "") const;
    int GetIfArgPrefixInt(const TStr& PrefixStr,
                          const int& DfVal = 0,
                          const TStr& DNm = "") const;
    TIntV GetIfArgPrefixIntV(const TStr& PrefixStr,
                             TIntV& DfValV,
                             const TStr& DNm = "") const;
    TIntV GetIfArgPrefixIntV(const TStr& PrefixStr,
                             const TStr& DNm = "") const {
        TIntV DfValV;
        return GetIfArgPrefixIntV(PrefixStr, DfValV, DNm);
    }
    TFltV GetIfArgPrefixFltV(const TStr& PrefixStr,
                             TFltV& DfValV,
                             const TStr& DNm = "") const;
    TFltV GetIfArgPrefixFltV(const TStr& PrefixStr,
                             const TStr& DNm = "") const {
        TFltV DfValV;
        return GetIfArgPrefixFltV(PrefixStr, DfValV, DNm);
    }
    double GetIfArgPrefixFlt(const TStr& PrefixStr,
                             const double& DfVal = 0,
                             const TStr& DNm = "") const;

    // environment variables
    static void PutVarVal(const TStr& VarNm,
                          const TStr& VarVal);
