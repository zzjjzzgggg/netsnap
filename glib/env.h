...
public:
  ...
  TEnv& operator=(const TEnv& Env){ /*Args=Env.Args;*/ ArgV=Env.ArgV; HdStr=Env.HdStr; MnArgs=Env.MnArgs; SilentP=Env.SilentP; Notify=Env.Notify; return *this;}
  ...
  TStr GetArg(const int& ArgN) const { IAssert((0<=ArgN)&&(ArgN<GetArgs())); return ArgV[ArgN]; }
  ...
  bool IsArgPrefix(const TStr& PrefixStr) const { return GetPrefixArgN(PrefixStr)!=-1;}
  TStr GetArgPostfix(const TStr& PrefixStr) const;

  // top level arguments handling
  void PrepArgs(const TStr& _HdStr=TStr(), const int& _MnArgs=1);
  ...
  TStrV GetIfArgPrefixStrV(const TStr& PrefixStr, const TStr& DNm="") const { TStrV DfValV; return GetIfArgPrefixStrV(PrefixStr, DfValV, DNm);}
