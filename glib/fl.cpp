...
TSOut& TSOut::operator<<(TSIn& SIn) {
    while (!SIn.Eof()) operator<<((char)SIn.GetCh());
    return *this;
}
...
int TFIn::GetFLen() const {
    ...
    EAssertR(fseek(FileId, 0, SEEK_END)==0,"Error seeking into file '"+GetSNm()+"'.");
 ...
 TSBase(TSStr(FNm.CStr())), FileId(NULL) {
    switch (FAccess){
        ...
        case faAppend:
          ...
          if (FileId!=NULL) fseek(FileId, SEEK_END, 0);
          break;
...
int TFInOut::PutBf(const void* LBf, const TSize& LBfL) {
    int LBfS = 0;
    for (TSize i = 0; i < LBfL; i++) LBfS += ((char *)LBf)[i];
...
TMIn::TMIn(const void* _Bf, const int& _BfL, const bool& TakeBf):
    ...
    if (TakeBf) Bf=(char*)_Bf;
    else Bf=new char[BfL]; memmove(Bf, _Bf, BfL);
}
...
void TFRnd::RefreshFPos(){
    EAssertR(fseek(FileId, 0, SEEK_CUR)==0,"Error seeking into file '"+TStr(FNm)+"'.");
}

TFRnd::TFRnd(const TStr& _FNm, const TFAccess& FAccess, const bool& CreateIfNo, const int& _HdLen, const int& _RecLen):
...
int TFRnd::GetFLen(){
    int FPos=GetFPos();
    EAssertR(fseek(FileId, 0, SEEK_END)==0,"Error seeking into file '"+TStr(FNm)+"'.");
...
void TFile::Rename(const TStr& SrcFNm, const TStr& DstFNm){
    EAssertR(rename(SrcFNm.CStr(), DstFNm.CStr())==0,"Error renaming file '"+SrcFNm+"' to "+DstFNm+"'.");
}
