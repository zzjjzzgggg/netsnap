...
double TSpecFunc::Normal(double x, const double mu,
                         const double sigma) {
  return 1 / (sqrt(2 * TMath::Pi) * sigma) *
         exp(-pow((x - mu) / sigma, 2) / 2);
}

double TSpecFunc::Binomial(const int K, const int N,
                           const double p) {
  return exp(lgamma(N + 1.0) - lgamma(N - K + 1.0) -
             lgamma(K + 1.0) + K * log(p) +
             (N - K) * log(1 - p));
}
 ...
 **/
double TSpecFunc::BetaBinomial(const int k, const int n,
                               const double alpha,
                               const double beta) {
  return exp(lgamma(n + 1.0) - lgamma(k + 1.0) -
             lgamma(n - k + 1.0) + lgamma(k + alpha) +
             lgamma(n - k + beta) - lgamma(n + alpha + beta) +
             lgamma(alpha + beta) - lgamma(alpha) -
             lgamma(beta));
}

/*gser*/
void TSpecFunc::GammaPSeries(double& gamser, const double& a,
  ...
  gln = LnGamma(a);
  if (x <= 0.0) {
    IAssert(x >= 0); /*if (x < 0.0) nrerror("x less than 0 in
                        routine gser");*/
    gamser = 0.0;
    return;
  } else {
    ...
    Fail; /*nrerror("a too large, ITMAX too small in routine
             gser");*/
    return;
  }
}

void TSpecFunc::GammaQContFrac /*gcf*/ (double& gammcf,
                                        const double& a,
                                        const double& x,
                                        double& gln) {
  static const int ITMAX = 100;
  ...
  IAssert(i <= ITMAX);
  /*if (i > ITMAX) nrerror("a too large, ITMAX too small in
   * gcf");*/
  gammcf = exp(-x + a * log(x) - (gln)) * h;
}

double TSpecFunc::GammaQ /*gammq*/ (const double& a,
                                    const double& x) {
  IAssert((x >= 0) && (a > 0));
...
double TSpecFunc::BetaCf(const double& a, const double& b,
                         const double& x) {
...
double TSpecFunc::BetaI(const double& a, const double& b,
                        const double& x) {
  double bt;
...
void TSpecFunc::LinearFit(const TVec<TFltPr>& XY, double& A,
                          double& B, double& SigA,
                          double& SigB, double& Chi2,
                          double& R2) {
  // y = a + bx :: SigA (SigB): A's uncertainty; Chi2: std dev
  // on all points
  int i;
  ...
  // calculate R squared
  {
    double N = XY.Len(), sXY = 0.0, sX = 0.0, sY = 0.0,
           sSqX = 0.0, sSqY = 0.0;
    ...
    R2 = TMath::Sqr(N * sXY - sX * sY) /
         ((N * sSqX - sX * sX) * (N * sSqY - sY * sY));
  }
...
void TSpecFunc::PowerFit(const TVec<TFltPr>& XY, double& A,
                         double& B, double& SigA,
                         double& SigB, double& Chi2,
                         double& R2) {
  // y = a x^b :: SigA (SigB): A's uncertainty; Chi2: std dev
  // on all points
  // log fit
  double AA, BB;
  TFltPrV LogXY(XY.Len(), 0);
  for (int s = 0; s < XY.Len(); s++) {
    LogXY.Add(TFltPr(log((double)XY[s].Val1),
                     log((double)XY[s].Val2)));
  }
...
void TSpecFunc::LogFit(const TVec<TFltPr>& XY, double& A,
                       double& B, double& SigA, double& SigB,
                       double& Chi2, double& R2) {
  // Y = A + B*log(X)
  TFltPrV LogXY(XY.Len(), 0);
  for (int s = 0; s < XY.Len(); s++) {
...
void TSpecFunc::ExpFit(const TVec<TFltPr>& XY, double& A,
                       double& B, double& SigA, double& SigB,
                       double& Chi2, double& R2) {
  // Y = A * exp(B*X)
...
// Entropy of a distribution: ValV[i] contains the number of
// events i
...
// Where 0 means the event did not occur, and 1 means it
// occured.
...
double TSpecFunc::EntropyBias(const double& B) {
  ...
  if (BToP.IsKey(TMath::Round(B, 3))) {
    return BToP.GetDat(TMath::Round(B, 3));
  } else {
    return -1.0;
  }
}
...
TMom::TMom(const TFltV& _ValV)
    :  // WgtV(_ValV.Len(), 0), ValV(_ValV.Len(), 0),
      ValWgtV(_ValV.Len(), 0),
      SumW(), ValSumW(), UsableP(false), UnusableVal(-1),
      Mn(), Mx(), Mean(), Vari(), SDev(), SErr(), Median(),
      Quart1(), Quart3(), DecileV(), PercentileV() {
  for (int ValN = 0; ValN < _ValV.Len(); ValN++) {
    Add(_ValV[ValN], 1);
  }
  Def();
}
...
void TMom::Def() {
  ...
  if (UsableP) {
    ...
    if (ValWgtV.Len() > 1) {
      ...
      Vari = Vari / SumW;
      SErr =
          sqrt(Vari / (ValWgtV.Len() * (ValWgtV.Len() - 1)));
    }
    // Standard-Deviation
    ...
    for (int ValN = 0; ValN < ValWgtV.Len(); ValN++) {
      ...
      if (CurSumW > 0.5 * SumW) {
        Median = ValWgtV[ValN].Val1;
        break;
      } else if (CurSumW == 0.5 * SumW) {
        Median = 0.5 * (ValWgtV[ValN].Val1 +
                        ValWgtV[ValN + 1].Val1);
        break;
      }
    }
    ...
    for (int ValN = 0; ValN < ValWgtV.Len(); ValN++) {
      ...
      if (CurSumW > SumW * DecileN * 0.1) {
        DecileV[DecileN] = ValWgtV[ValN].Val1;
        DecileN++;
      }
      if (CurSumW > SumW * PercentileN * 0.01) {
        PercentileV[PercentileN] = ValWgtV[ValN].Val1;
        PercentileN++;
      }
    }
  }
...
double TMom::GetByNm(const TStr& MomNm) const {
  if (MomNm == "Mean") {
    ...
    return GetSErr();
  } else if (MomNm == "Median") {
    return GetMedian();
  } else if (MomNm == "Quart1") {
    return GetQuart1();
  } else if (MomNm == "Quart3") {
    return GetQuart3();
  } else if (MomNm == "Decile0") {
    return GetDecile(0);
  } else if (MomNm == "Decile1") {
    return GetDecile(1);
  } else if (MomNm == "Decile2") {
    return GetDecile(2);
  } else if (MomNm == "Decile3") {
    return GetDecile(3);
  } else if (MomNm == "Decile4") {
    return GetDecile(4);
  } else if (MomNm == "Decile5") {
    return GetDecile(5);
  } else if (MomNm == "Decile6") {
    return GetDecile(6);
  } else if (MomNm == "Decile7") {
    return GetDecile(7);
  } else if (MomNm == "Decile8") {
    return GetDecile(8);
  } else if (MomNm == "Decile9") {
    return GetDecile(9);
  } else if (MomNm == "Decile10") {
    return GetDecile(10);
  } else {
    Fail;
    return 0;
...
TStr TMom::GetStr(const char& SepCh, const char& DelimCh,
                  const bool& DecileP,
                  const bool& PercentileP,
                  const TStr& FmtStr) const {
  TChA ChA;
  if (IsUsable()) {
    ChA += "[";
    ChA += SepCh;
    ChA += "Vals";
    ChA += DelimCh;
    ChA += TInt::GetStr(GetVals());
    ChA += SepCh;
    ChA += "Min";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetMn(), FmtStr.CStr());
    ChA += SepCh;
    ChA += "Max";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetMx(), FmtStr.CStr());
    ChA += SepCh;
    ChA += "Mean";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetMean(), FmtStr.CStr());
    ChA += SepCh;
    // ChA+="Vari"; ChA+=DelimCh; ChA+=TFlt::GetStr(GetVari(),
    // FmtStr.CStr()); ChA+=SepCh;
    ChA += "SDev";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetSDev(), FmtStr.CStr());
    ChA += SepCh;
    // ChA+="SErr"; ChA+=DelimCh; ChA+=TFlt::GetStr(GetSErr(),
    // FmtStr.CStr()); ChA+=SepCh;
    ChA += "Quart1";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetQuart1(), FmtStr.CStr());
    ChA += SepCh;
    ChA += "Median";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetMedian(), FmtStr.CStr());
    ChA += SepCh;
    ChA += "Quart3";
    ChA += DelimCh;
    ChA += TFlt::GetStr(GetQuart3(), FmtStr.CStr());
    ChA += SepCh;
    if (DecileP) {
      for (int DecileN = 0; DecileN <= 10; DecileN++) {
        ChA += "Dec";
        ChA += TInt::GetStr(DecileN);
        ChA += DelimCh;
        ChA +=
            TFlt::GetStr(GetDecile(DecileN), FmtStr.CStr());
        ChA += SepCh;
      }
    }
    if (PercentileP) {
      for (int PercentileN = 0; PercentileN <= 100;
           PercentileN++) {
        ChA += "Per";
        ChA += TInt::GetStr(PercentileN);
        ChA += DelimCh;
        ChA += TFlt::GetStr(GetPercentile(PercentileN),
                            FmtStr.CStr());
        ChA += SepCh;
      }
    }
...
TStr TMom::GetNmVStr(const TStr& VarPfx, const char& SepCh,
                     const bool& DecileP,
                     const bool& PercentileP) {
  TChA ChA;
  ChA += VarPfx;
...
TStr TMom::GetValVStr(const char& SepCh, const bool& DecileP,
                      const bool& PercentileP) const {
  TChA ChA;
  if (IsUsable()) {
    ChA += TInt::GetStr(GetVals());
    ChA += SepCh;
    ChA += TFlt::GetStr(GetMn());
    ChA += SepCh;
    ChA += TFlt::GetStr(GetMx());
    ChA += SepCh;
    ChA += TFlt::GetStr(GetMean());
    ChA += SepCh;
    // ChA+=TFlt::GetStr(GetVari()); ChA+=SepCh;
    ChA += TFlt::GetStr(GetSDev());
    ChA += SepCh;
    // ChA+=TFlt::GetStr(GetSErr()); ChA+=SepCh;
    ChA += TFlt::GetStr(GetQuart1());
    ChA += SepCh;
    ChA += TFlt::GetStr(GetMedian());
    ChA += SepCh;
    ChA += TFlt::GetStr(GetQuart3());
    ChA += SepCh;
    if (DecileP) {
      for (int DecileN = 0; DecileN <= 10; DecileN++) {
        ChA += TFlt::GetStr(GetDecile(DecileN));
        ChA += SepCh;
      }
    }
    if (PercentileP) {
...
TCorr::TCorr(const TFltV& ValV1, const TFltV& ValV2)
  ...
  if (sxx * syy == 0) {
    CorrCf = 0;  //** not in numerical recipes - check why
                 //(pojavi se, ko so same nicle)
  } else {
    CorrCf = sxy / sqrt(sxx * syy);
  }
  // calculate correlation coefficient significance level
  double df = ValVLen - 2;
  double t = CorrCf * sqrt(df / ((1.0 - CorrCf + TINY) *
                                 (1.0 + CorrCf + TINY)));
  CorrCfPrb =
      TSpecFunc::BetaI(0.5 * df, 0.5, df / (df + t * t));
  // calculate Fisher's Z transformation
  FisherZ = 0.5 * log((1.0 + (CorrCf) + TINY) /
                      (1.0 - (CorrCf) + TINY));
}

/////////////////////////////////////////////////
// Statistical Tests
void TStatTest::AveVar(const TFltV& ValV, double& Ave,
                       double& Var) {
  Ave = 0;
...
double TStatTest::KsProb(const double& Alam) {
  ...
  double a2 = -2.0 * Alam * Alam, fac = 2.0, sum = 0.0, term,
         termbf = 0.0;
  for (int j = 1; j <= 100; j++) {
    term = fac * exp(a2 * j * j);
    sum += term;
...
void TStatTest::ChiSquareOne(const TFltV& ObservedBinV,
  ...
  SignificancePrb = TSpecFunc::GammaQ(
      0.5 * (DegreesOfFreedom), 0.5 * (ChiSquareVal));
}
...
void TStatTest::ChiSquareTwo(const TFltV& ObservedBin1V,
  ...
  for (int BinN = 1; BinN <= Bins; BinN++) {
    if ((ObservedBin1V[BinN] == 0.0) &&
        (ObservedBin2V[BinN] == 0.0)) {
      DegreesOfFreedom--;
    } else {
      double BinDiff =
          ObservedBin1V[BinN] - ObservedBin2V[BinN];
      ChiSquareVal +=
          BinDiff * BinDiff /
          (ObservedBin1V[BinN] + ObservedBin2V[BinN]);
    }
...
void TStatTest::TTest(const TFltV& ValV1, const TFltV& ValV2,
                      double& TTestVal, double& TTestPrb) {
  ...
  double df = TMath::Sqr(var1 / n1 + var2 / n2) /
              (TMath::Sqr(var1 / n1) / (n1 - 1) +
               TMath::Sqr(var2 / n2) / (n2 - 1));
  TTestPrb = TSpecFunc::BetaI(
      0.5 * df, 0.5, df / (df + TMath::Sqr(TTestVal)));
}

void TStatTest::KsTest(const TFltV& ValV1, const TFltV& ValV2,
...
void TStatTest::KsTest(const TFltPrV& ValCntV1,
                       const TFltPrV& ValCntV2, double& DStat,
                       double& PVal) {
  IAssert(ValCntV1.IsSorted() && ValCntV2.IsSorted());
  int i1 = 0, i2 = 0;
  double N1 = 0.0, N2 = 0.0, CumSum1 = 0.0, CumSum2 = 0.0,
         Cdf1 = 0.0, Cdf2 = 0.0;
  DStat = 0.0;
  PVal = 0.0;
  for (int i = 0; i < ValCntV1.Len(); i++)
    N1 += ValCntV1[i].Val2;
  for (int i = 0; i < ValCntV2.Len(); i++)
    N2 += ValCntV2[i].Val2;
  if (!(N1 > 0.0 && N2 > 0.0)) {
    DStat = 1.0;
    PVal = 0.0;
  ...
  if (ItemV.Len() == 0) {
    ...
    if (ItemV.Last() == Items - 1) {
      ...
      if (OrderN < 0) {
        ...
        ItemV[OrderN]++;
        for (int SubOrderN = OrderN + 1; SubOrderN < Order;
             SubOrderN++) {
...
/////////////////////////////////////////////////
// Linear-Regression
PLinReg TLinReg::New(const TFltVV& _XVV, const TFltV& _YV,
                     const TFltV& _SigV) {
...
void TLinReg::NR_covsrt(TFltVV& CovarVV, const int& Vars,
                        const TIntV& ia, const int& mfit) {
  ...
  for (int j = Vars; j >= 1; j--) {
    if (ia[j] != 0) {
      for (int i = 1; i <= Vars; i++) {
        Swap(CovarVV.At(i, k), CovarVV.At(i, j));
      }
      {
        for (int i = 1; i <= Vars; i++) {
          Swap(CovarVV.At(k, i), CovarVV.At(j, i));
        }
      }
      k--;
    }
...
void TLinReg::NR_gaussj(TFltVV& a, const int& n, TFltVV& b,
                        const int& m) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    a.At(icol, icol) = 1.0;
    for (l = 1; l <= n; l++) {
      a.At(icol, l) = a.At(icol, l) * pivinv;
    }
    for (l = 1; l <= m; l++) {
      b.At(icol, l) = b.At(icol, l) * pivinv;
    }
    for (ll = 1; ll <= n; ll++) {
      if (ll != icol) {
        dum = a.At(ll, icol);
...
void TLinReg::NR_lfit() {
  ...
  if (mfit == 0) {
    TExcept::Throw("No parameters to be fitted in LFit");
  }
  for (j = 1; j <= mfit; j++) {
    for (k = 1; k <= mfit; k++) {
      CovarVV.At(j, k) = 0.0;
...
PSvd TSvd::New(const TFltVV& _XVV, const TFltV& _YV,
               const TFltV& _SigV) {
...
double TSvd::NR_pythag(double a, double b) {
  ...
  if (absa > absb) {
    ...
    return (absb == 0.0
                ? 0.0
                : absb * sqrt(1.0 + TMath::Sqr(absa / absb)));
  }
}

void TSvd::NR_svdcmp(TFltVV& a, int m, int n, TFltV& w,
                     TFltVV& v) {
  ...
  for (i = 1; i <= n; i++) {
    ...
    anorm = NR_FMAX(
        anorm, (fabs(double(w[i])) + fabs(double(rv1[i]))));
  }
  for (i = n; i >= 1; i--) {
    if (i < n) {
      if (g) {
  ...
  for (k = n; k >= 1; k--) {
    for (its = 1; its <= 30; its++) {
      ...
      for (l = k; l >= 1; l--) {
        ...
        if ((double)(fabs(double(w[nm])) + anorm) == anorm)
          break;
      }
      if (flag) {
        c = 0.0;
...
void TSvd::NR_svbksb(TFltVV& u, TFltV& w, TFltVV& v, int m,
                     int n, TFltV& b, TFltV& x) {
  int jj, j, i;
  double s;
...
void TSvd::NR_svdvar(TFltVV& v, int ma, TFltV& w,
                     TFltVV& cvm) {
  int k, j, i;
  double sum;
...
void TSvd::Svd(const TFltVV& InMtx, TFltVV& LSingV,
               TFltV& SingValV, TFltVV& RSingV) {
  // LSingV = InMtx;
  ...
  TSvd::NR_svdcmp(LSingV, InMtx.GetXDim(), InMtx.GetYDim(),
                  SingValV, RSingV);
...
void TSvd::Svd1Based(const TFltVV& InMtx1, TFltVV& LSingV,
                     TFltV& SingValV, TFltVV& RSingV) {
  LSingV = InMtx1;
  SingValV.Gen(InMtx1.GetYDim());
  RSingV.Gen(InMtx1.GetYDim(), InMtx1.GetYDim());
  TSvd::NR_svdcmp(LSingV, InMtx1.GetXDim() - 1,
                  InMtx1.GetYDim() - 1, SingValV, RSingV);
  // 0-th singular value/vector is full of zeros, delete it
  SingValV.Del(0);
  LSingV.DelX(0);
  LSingV.DelY(0);
