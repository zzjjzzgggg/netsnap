/////////////////////////////////////////////////
// Environment
TEnv::TEnv(const int& _Args, char** _ArgV,
...
void TEnv::PrepArgs(const TStr& _HdStr, const int& _MnArgs) {
    ...
    SilentP = true;
    SilentP =
        Env.GetIfArgPrefixBool("-silent:", false, "Silence");
    // start header
    if (!SilentP) {
        // print header
        if (!HdStr.Empty()) {
            ...
            // print header-line
            for (int ChN = 0;
                 ChN < HdStr.Len() + DateStr.Len() + 3;
                 ChN++) {
                printf("=");
            }
            printf("\n");
        }
        // print start of 'usage' message if not enough
        // arguments
...
bool TEnv::IsEndOfRun() const {
    if (!SilentP) {
        ...
        if (HdStr.Empty()) {
            ...
            for (int ChN = 0; ChN < HdStr.Len(); ChN++) {
                printf("=");
            }
            printf("\n");
        }
