...
int TSs::GetXLen() const {
	if (CellStrVV.Len() == 0)return 0;
	else {
		int MxXLen = CellStrVV[0]->Len();
		for (int Y = 1; Y < CellStrVV.Len(); Y++) MxXLen = TInt::GetMx(MxXLen, CellStrVV[Y]->Len());
		return MxXLen;
	}
}

int TSs::GetXLen(const int& Y) const {
	if ((0 <= Y) && (Y < CellStrVV.Len()))	return CellStrVV[Y]->Len();
	else return 0;
}
...
void TSs::DelX(const int& X) {
	int YLen = GetYLen();
	for (int Y = 0; Y < YLen; Y++)	CellStrVV[Y]->V.Del(X);
}
	...
	if (!SIn->Eof()) {
		...
		while (!SIn->Eof()) {
			...
			if (IsQStr && (Ch == '"')) {
				...
				Ch = SIn->GetCh();
				forever{
					while ((!SIn->Eof())&&(Ch!='"')) ChA+=Ch; Ch=SIn->GetCh();
					if (Ch=='"') {
						Ch=SIn->GetCh();
...
TSsFmt TSs::GetSsFmtFromStr(const TStr& SsFmtNm) {
	...
	if (LcSsFmtNm == "tab") {
		return ssfTabSep;
	} else if (LcSsFmtNm == "comma") {
		return ssfCommaSep;
	} else if (LcSsFmtNm == "semicolon") {
		return ssfSemicolonSep;
	} else if (LcSsFmtNm == "space") {
		return ssfSpaceSep;
	} else if (LcSsFmtNm == "white") {
		return ssfWhiteSep;
	} else {
		return ssfUndef;
	}
