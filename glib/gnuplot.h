...
private:
	...
	public:
		TGpSeries() :
				SeriesTy(gpwLines), XYValV(), ZValV(), Label(), WithStyle(), DataFNm(), XCol(
						0), YCol(0), ZCol(0) {
		}
...
public:
	...
	int AddPlot(const THash<TKey, TDat, THashFunc>& XYValH, const TGpSeriesTy& SeriesTy = gpwLinesPoints, const TStr& Label = TStr(), const TStr& Style = TStr()) {
		TFltKdV XYFltValV(XYValH.Len(), 0);
		for (int k = XYValH.FFirstKeyId(); XYValH.FNextKeyId(k);)
			XYFltValV.Add(TFltKd(TFlt(XYValH.GetKey(k)), TFlt(XYValH[k])));
		XYFltValV.Sort();
		return AddPlot(XYFltValV, SeriesTy, Label, Style);
	}
	template<class TKey, class THashFunc>
	int AddPlot(THash<TKey, TMom, THashFunc>& ValMomH, const TGpSeriesTy& SeriesTy = gpwLinesPoints, const TStr& Label = TStr(), const TStr& Style = TStr(), bool PlotAvg = true,
			bool PlotMed = true, bool PlotMin = false, bool PlotMax = false, bool PlotSDev = false, bool PlotStdErr = false);
	...
	void SavePng(const int& SizeX = 1000, const int& SizeY = 800, const TStr& Comment = TStr()) {SavePng(PlotFNm.GetFMid() + ".png", SizeX, SizeY, Comment);}
	void SavePng(const TStr& FNm, const int& SizeX = 1000, const int& SizeY =800, const TStr& Comment = TStr(), const TStr& Terminal = TStr());
	void SaveEps(const int& FontSz = 30, const TStr& Comment = TStr()) {SaveEps(PlotFNm.GetFMid() + ".eps", FontSz, Comment);}
	...
	template<class TVal1, class TVal2>
	static void PlotValV(const TVec<TPair<TVal1, TVal2> >& ValV1,
			const TStr& Name1, const TVec<TPair<TVal1, TVal2> >& ValV2,
			const TStr& Name2, const TStr& OutFNmPref, const TStr& Desc = "",
			const TStr& XLabel = "", const TStr& YLabel = "",
			const TGpScaleTy& ScaleTy = gpsAuto, const bool& PowerFit = false,
			const TGpSeriesTy& SeriesTy = gpwLinesPoints);
	template<class TVal1>
	static void PlotValV(const TVec<TVal1>& ValV, const TStr& OutFNmPref,
			const TStr& Desc = "", const TStr& XLabel = "", const TStr& YLabel = "", const TGpScaleTy& ScaleTy = gpsAuto,
			const bool& PowerFit = false, const TGpSeriesTy& SeriesTy = gpwLinesPoints);
	template<class TKey, class TVal, class THashFunc>
	static void PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH,
			const TStr& OutFNmPref, const TStr& Desc = "", const TStr& XLabel = "", const TStr& YLabel = "", const TGpScaleTy& ScaleTy =	gpsAuto, const bool& PowerFit = false,
			const TGpSeriesTy& SeriesTy = gpwLinesPoints, const bool& PlotNCDF = false, const bool& ExpBucket = false);
	template<class TKey, class TVal, class THashFunc>
	static void PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH1,
			const TStr& Label1, const THash<TKey, TVal, THashFunc>& ValCntH2,
			const TStr& Label2, const TStr& OutFNmPref, const TStr& Desc,
			const TStr& XLabel, const TStr& YLabel, const TGpScaleTy& ScaleTy = gpsAuto, const TGpSeriesTy& SeriesTy = gpwLinesPoints);
	template<class TKey, class TVal, class THashFunc>
	static void PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH1,
			const TStr& Label1, const THash<TKey, TVal, THashFunc>& ValCntH2,
			const TStr& Label2, const THash<TKey, TVal, THashFunc>& ValCntH3,
			const TStr& Label3, const TStr& OutFNmPref, const TStr& Desc,
			const TStr& XLabel, const TStr& YLabel, const TGpScaleTy& ScaleTy = gpsAuto, const TGpSeriesTy& SeriesTy = gpwLinesPoints);
	template<class TVal1>
	static void PlotValMomH(THash<TVal1, TMom>& ValMomH, const TStr& OutFNmPref,
			const TStr& Desc = "", const TStr& XLabel = "", const TStr& YLabel = "", const TGpScaleTy& ScaleTy = gpsAuto,
			const TGpSeriesTy& SeriesTy = gpwLinesPoints, bool PlotAvg = true,
			bool PlotMed = true, bool PlotMin = false, bool PlotMax = false,
			bool PlotSDev = false, bool PlotStdErr = true);
...
template<class TVal1, class TVal2>
void TGnuPlot::SaveTs(const TVec<TPair<TVal1, TVal2> >& ValV, const TStr& FNm,
		const TStr& HeadLn) {
...
void TGnuPlot::SaveTs(const TVec<TTriple<TVal1, TVal2, TVal3> >& ValV,
		const TStr& FNm, const TStr& HeadLn) {
	FILE *F = fopen(FNm.CStr(), "wt");
	EAssert(F);
	if (!HeadLn.Empty()) {
		fprintf(F, "# %s\n", HeadLn.CStr());
...
template<class TVal, int Vals>
void TGnuPlot::SaveTs(const TVec<TTuple<TVal, Vals> >& ValV, const TStr& FNm,
		const TStr& HeadLn) {
	FILE *F = fopen(FNm.CStr(), "wt");
	EAssert(F);
	if (!HeadLn.Empty()) {
...
template<class TKey, class THashFunc>
int TGnuPlot::AddPlot(THash<TKey, TMom, THashFunc>& ValMomH, const TGpSeriesTy& SeriesTy, const TStr& Label,
		const TStr& Style, bool PlotAvg, bool PlotMed, bool PlotMin, bool PlotMax, bool PlotSDev, bool PlotStdErr) {
	...
	for (int i = ValMomH.FFirstKeyId(); ValMomH.FNextKeyId(i);) {
		...
		if (PlotAvg) {
			if (PlotSDev)
				AvgV.Add(TFltTr((double) ValMomH.GetKey(i), ValMomH[i].GetMean(), ValMomH[i].GetSDev()));
			else // std deviation
				AvgV2.Add(TFltPr((double) ValMomH.GetKey(i), ValMomH[i].GetMean()));
			if (PlotStdErr) StdErrV.Add(TFltTr((double) ValMomH.GetKey(i), ValMomH[i].GetMean(), ValMomH[i].GetSDev() / sqrt((double) ValMomH[i].GetVals())));
		}
	...
	AvgV.Sort(); AvgV2.Sort(); MedV.Sort(); MinV.Sort(); MaxV.Sort();
	int PlotId = 0;
...
void TGnuPlot::PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH,
		...
		const TGpSeriesTy& SeriesTy, const bool& PlotNCDF,
		const bool& ExpBucket) {
	TFltPrV IdCntV(ValCntH.Len(), 0), BucketV;
	for (int i = ValCntH.FFirstKeyId(); ValCntH.FNextKeyId(i);) IdCntV.Add(TFltPr(double(ValCntH.GetKey(i)), double(ValCntH[i])));
	IdCntV.Sort();
	if (ExpBucket) {
	...
	if (PlotNCDF) {
		...
		for (int i = NCdfV.Len() - 2; i >= 0; i--) NCdfV[i].Val2 = NCdfV[i].Val2 + NCdfV[i + 1].Val2;
		PlotValV(NCdfV, OutFNmPref, Desc, "NCDF " + XLabel, "NCDF " + YLabel, ScaleTy, PowerFit, SeriesTy);
	} else PlotValV(IdCntV, OutFNmPref, Desc, XLabel, YLabel, ScaleTy, PowerFit, SeriesTy);
}
...
void TGnuPlot::PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH1,
		const TStr& Label1, const THash<TKey, TVal, THashFunc>& ValCntH2,
		const TStr& Label2, const TStr& OutFNmPref, const TStr& Desc,
		const TStr& XLabel, const TStr& YLabel, const TGpScaleTy& ScaleTy,
		const TGpSeriesTy& SeriesTy) {
	PlotValCntH(ValCntH1, Label1, ValCntH2, Label2,	THash<TKey, TVal, THashFunc>(), "", OutFNmPref, Desc, XLabel,	YLabel, ScaleTy, SeriesTy);
}
...
void TGnuPlot::PlotValCntH(const THash<TKey, TVal, THashFunc>& ValCntH1,
	...
	for (int i = ValCntH1.FFirstKeyId(); ValCntH1.FNextKeyId(i);) IdCntV1.Add(TFltPr(double(ValCntH1.GetKey(i)), double(ValCntH1[i])));
	for (int i = ValCntH2.FFirstKeyId(); ValCntH2.FNextKeyId(i);) IdCntV2.Add(TFltPr(double(ValCntH2.GetKey(i)), double(ValCntH2[i])));
	for (int i = ValCntH3.FFirstKeyId(); ValCntH3.FNextKeyId(i);) IdCntV3.Add(TFltPr(double(ValCntH3.GetKey(i)), double(ValCntH3[i])));
	IdCntV1.Sort();
	IdCntV2.Sort();
	IdCntV3.Sort();
...
void TGnuPlot::PlotValV(const TVec<TPair<TVal1, TVal2> >& ValV1,
		...
		const TStr& Name2, const TStr& OutFNmPref, const TStr& Desc,
		const TStr& XLabel, const TStr& YLabel, const TGpScaleTy& ScaleTy,
		const bool& PowerFit, const TGpSeriesTy& SeriesTy) {
	TFltKdV IdCntV1(ValV1.Len(), 0), IdCntV2(ValV2.Len(), 0);
	for (int i = 0; i < ValV1.Len(); i++) IdCntV1.Add(TFltKd(double(ValV1[i].Val1), double(ValV1[i].Val2)));
	for (int i = 0; i < ValV2.Len(); i++) IdCntV2.Add(TFltKd(double(ValV2[i].Val1), double(ValV2[i].Val2)));
	if (IdCntV1.Empty() || IdCntV2.Empty()) {
		printf("*** Empty plot %s\n", OutFNmPref.CStr());
		return;
	}
	IdCntV1.Sort();
...
template<class TVal1>
void TGnuPlot::PlotValV(const TVec<TVal1>& ValV, const TStr& OutFNmPref,
		const TStr& Desc, const TStr& XLabel, const TStr& YLabel,
		const TGpScaleTy& ScaleTy, const bool& PowerFit,
		const TGpSeriesTy& SeriesTy) {
	TFltKdV IdCntV(ValV.Len(), 0);
	for (int i = 0; i < ValV.Len(); i++) IdCntV.Add(TFltKd(double(i + 1), double(ValV[i])));
	if (IdCntV.Empty()) {
		printf("*** Empty plot %s\n", OutFNmPref.CStr());
		return;
	}
...
template<class TVal1>
void TGnuPlot::PlotValMomH(THash<TVal1, TMom>& ValMomH, const TStr& OutFNmPref,
		const TStr& Desc, const TStr& XLabel, const TStr& YLabel,
		const TGpScaleTy& ScaleTy, const TGpSeriesTy& SeriesTy, bool PlotAvg,
		bool PlotMed, bool PlotMin, bool PlotMax, bool PlotSDev, bool PlotStdErr) {
	TFltTrV AvgV, StdErrV;
	TFltPrV AvgV2, MedV, MinV, MaxV;
	for (int i = ValMomH.FFirstKeyId(); ValMomH.FNextKeyId(i);) {
		ValMomH[i].Def();
		if (PlotAvg) {
			if (PlotSDev) AvgV.Add(TFltTr((double) ValMomH.GetKey(i), ValMomH[i].GetMean(), ValMomH[i].GetSDev()));
			// std deviation
			else AvgV2.Add(TFltPr((double) ValMomH.GetKey(i),	ValMomH[i].GetMean()));
			if (PlotStdErr) StdErrV.Add(TFltTr((double) ValMomH.GetKey(i), ValMomH[i].GetMean(), ValMomH[i].GetSDev() / sqrt((double) ValMomH[i].GetVals())));
		}
		if (PlotMed) MedV.Add(TFltPr((double) ValMomH.GetKey(i), ValMomH[i].GetMedian()));
		if (PlotMin) MinV.Add(TFltPr((double) ValMomH.GetKey(i), ValMomH[i].GetMn()));
