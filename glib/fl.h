...
public:
    ...
    void* LoadNewBf(const int& BfL){void* Bf=(void*)new char[BfL]; Cs+=GetBf(Bf, BfL); return Bf;}
    ...
    void Load(char*& CStr, const int& MxCStrLen, const int& CStrLen){CStr=new char[MxCStrLen+1]; Cs+=GetBf(CStr, CStrLen+1);}
    void Load(char*& CStr);
...
template <class T>
TSIn& operator>>(TSIn& SIn, T& Val) {Val.Load(SIn); return SIn;}
...
public:
	...
	char PeekCh(){int Ch=getchar(); ungetc(Ch, stdin); return char(Ch);}
...
// faCreate: "w+b" allows read and write, create the file first if it does not exists
// faUpdate: "r+b" allows read and write
// faAppend: "r+b"
// faRdOnly: "rb"
...
public:
    ...
    TStr GetFNm() const;
    void SetHdRecLen(const int& _HdLen, const int& _RecLen) { HdLen=_HdLen; RecLen=_RecLen; RecAct=(HdLen>=0)&&(RecLen>0); }
    ...
    void GetHd(void* Hd) { IAssert(RecAct); int FPos=GetFPos(); SetFPos(0); GetBf(Hd, HdLen); SetFPos(FPos); }
    void PutHd(const void* Hd) { IAssert(RecAct); int FPos=GetFPos(); SetFPos(0); PutBf(Hd, HdLen); SetFPos(FPos); }
    void GetRec(void* Rec, const int& RecN=-1) { IAssert(RecAct); if (RecN!=-1) { SetRecN(RecN);} GetBf(Rec, RecLen); }
    void PutRec(const void* Rec, const int& RecN=-1) { IAssert(RecAct); if (RecN!=-1) { SetRecN(RecN);} PutBf(Rec, RecLen); }
    ...
    void PutUInt64(const uint64& UInt) { PutBf(&UInt, sizeof(UInt)); }
    uint64 GetUInt64() { uint64 UInt; GetBf(&UInt, sizeof(UInt)); return UInt; }
