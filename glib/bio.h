...
namespace BIO {
  ...
  template <class TVal>
  void SaveVec(const TVec<TVal> &Vec, const TStr &Fnm,
               const TStr &Fmt, const TStr &anno);
  ...
  void SaveVec(const TVec<TVal> &Vec, const TStr &Fnm,
               const TStr &Fmt, const TStr &anno) {
    PSOut FOutPt = TZipOut::IsZipFNm(Fnm) ? TZipOut::New(Fnm)
                                          : TFOut::New(Fnm);
  ...
  void SavePrV(const TVec<TPair<TVal1, TVal2>> &Pairs,
    ...
    for (int i = 0; i < Pairs.Len(); i++)
      FOutPt->PutStrLn(TStr::Fmt(
          Fmt.CStr(), Pairs[i].Val1.Val, Pairs[i].Val2.Val));
  }
  ...
  void LoadIntV(const TStr &fnm, TIntV &dat,
                const int col = 0);
  void LoadIntSet(const TStr &fnm, TIntSet &dat,
                  const int col = 0);
  void LoadFltV(const TStr &fnm, TFltV &dat,
                const int col = 0);
  ...
  void LoadIntPrSet(const TStr &fnm, TIntPrSet &dat,
                    const int c1 = 0, const int c2 = 1);
  void LoadIntPrV(const TStr &fnm, TIntPrV &dat,
                  const int c1 = 0, const int c2 = 1);
