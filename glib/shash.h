...
private:
  TSetKey& GetSetKey(const int& KeyId) {TSetKey& SetKey=KeyV[KeyId];Assert(SetKey.HashCd!=-1); return SetKey; }
  const TSetKey& GetSetKey(const int& KeyId) const {const TSetKey& SetKey=KeyV[KeyId];Assert(SetKey.HashCd!=-1); return SetKey; }
...
public:
  ...
  void Load(TSIn& SIn) {PortV.Load(SIn); KeyV.Load(SIn);AutoSizeP=TBool(SIn); FFreeKeyId=TInt(SIn); FreeKeys=TInt(SIn);SIn.LoadCs(); }
  void Save(TSOut& SOut) const {PortV.Save(SOut);KeyV.Save(SOut);AutoSizeP.Save(SOut);FFreeKeyId.Save(SOut);FreeKeys.Save(SOut);SOut.SaveCs();}
  void LoadXml(const PXmlTok& XmlTok, const TStr& Nm="") {XLoadHd(Nm); TVec<TSetKey> KeyV; XLoad(KeyV); XLoad(AutoSizeP);for (int KeyN=0; KeyN<KeyV.Len(); KeyN++){AddKey(KeyV[KeyN].Key); }}
  void SaveXml(TSOut& SOut, const TStr& Nm) {Defrag(); XSaveHd(Nm); XSave(KeyV); XSave(AutoSizeP); }

  THashSet& operator=(const THashSet& Set) {if (this!=&Set) {PortV=Set.PortV; KeyV=Set.KeyV; AutoSizeP=Set.AutoSizeP;FFreeKeyId=Set.FFreeKeyId; FreeKeys=Set.FreeKeys; }return *this; }
  ...
  ::TSize GetMemUsed() const {return PortV.GetMemUsed() + KeyV.GetMemUsed() + sizeof(bool) + 2*sizeof(int); }

  TIter BegI() const {if (Len()>0) {if (IsKeyIdEqKeyN()) { return TIter(KeyV.BegI(), KeyV.EndI()); }int FKeyId=-1;  FNextKeyId(FKeyId);return TIter(KeyV.BegI()+FKeyId, KeyV.EndI()); }return TIter(KeyV.EndI(), KeyV.EndI());}
  ...
  void Gen(const int& ExpectVals) {PortV.Gen(GetNextPrime(ExpectVals/2)); KeyV.Gen(ExpectVals, 0);FFreeKeyId=-1; FreeKeys=0; PortV.PutAll(TInt(-1)); }
