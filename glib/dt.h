...
public:
	TRnd(const int& _Seed=0, const int& Steps=0){ SetSeed(_Seed); Move(Steps); }
	...
	void Randomize() { SetSeed(RndSeed); }
	void Move(const int& Steps){for (int StepN=0; StepN<Steps; StepN++) GetNextSeed(); }
	bool Check();

	static double GetUniDevStep(const int& Seed, const int& Steps){TRnd Rnd(Seed); Rnd.Move(Steps); return Rnd.GetUniDev(); }
	static double GetNrmDevStep(const int& Seed, const int& Steps){TRnd Rnd(Seed); Rnd.Move(Steps); return Rnd.GetNrmDev(); }
	static double GetExpDevStep(const int& Seed, const int& Steps){TRnd Rnd(Seed); Rnd.Move(Steps); return Rnd.GetExpDev(); }
...
public:
	...
	TMem(const TMem& Mem): MxBfL(Mem.MxBfL), BfL(Mem.BfL), Bf(NULL){if(MxBfL>0){Bf=new char[MxBfL]; memcpy(Bf, Mem.Bf, BfL); }}
	...
	explicit TMem(TSIn& SIn){ SIn.Load(MxBfL); SIn.Load(BfL); Bf=new char[MxBfL=BfL]; SIn.LoadBf(Bf, BfL); }

	void Save(TSOut& SOut) const { SOut.Save(MxBfL); SOut.Save(BfL); SOut.SaveBf(Bf, BfL); }
	void LoadXml(const PXmlTok& XmlTok, const TStr& Nm);
	void SaveXml(TSOut& SOut, const TStr& Nm) const;
	...
	void Gen(const int& _BfL){Clr(); Resize(_BfL); BfL=_BfL; }
	...
	void Trunc(const int& _BfL){if(0<=_BfL && _BfL<=BfL) BfL=_BfL; }
	...
	PSIn GetSIn() const {TMOut MOut(BfL); MOut.SaveBf(Bf, BfL); return MOut.GetSIn(); }

	static void LoadMem(const PSIn& SIn, TMem& Mem){Mem.Clr(); Mem.Gen(SIn->Len()); SIn->GetBf(Mem.Bf, SIn->Len()); }
	static void LoadMem(const PSIn& SIn, const PMem& Mem){Mem->Clr(); Mem->Gen(SIn->Len()); SIn->GetBf(Mem->Bf, SIn->Len()); }
...
public:
	...
	static PSOut New(const PMem& Mem){ return new TMemOut(Mem); }
	~TMemOut(){}
	int PutCh(const char& Ch){Mem->operator+=(Ch); return Ch; }
...
public:
  ...
  TRStr(const int& Len){IAssert(Len>=0); Refs=0; Bf=new char[Len+1]; Bf[Len]=0; }
  TRStr(const char* CStr){Refs=0; Bf=new char[strlen(CStr)+1]; strcpy(Bf, CStr); }
  TRStr(const char* CStr, const int& MxLen){Refs=0; Bf=new char[MxLen+1]; strncpy(Bf, CStr, MxLen); Bf[MxLen]=0; }
  TRStr(const char* CStr1, const char* CStr2){
    Refs=0; int CStr1Len=int(strlen(CStr1));
  ...
  void PutCh(const int& ChN, const char& Ch){Assert((0<=ChN)&&(ChN<Len())); Bf[ChN]=Ch; }
  char GetCh(const int& ChN) const {Assert((0<=ChN)&&(ChN<Len())); return Bf[ChN]; }
  ...
  static TRStr* GetNullRStr(){static TRStr NullRStr; Assert(NullRStr.Bf!=NULL); return &NullRStr; }
};
...
private:
	TRStr* RStr;
private:
	TStr(const char& Ch, bool){RStr=new TRStr(Ch); RStr->MkRef(); }
	TStr(const char& Ch1, const char& Ch2, bool){RStr=new TRStr(Ch1, Ch2); RStr->MkRef(); }
...
public:
	...
	TStr(const TMem& Mem){ RStr=new TRStr(Mem.Len()); RStr->MkRef(); memcpy(CStr(), Mem(), Mem.Len()); Optimize(); }
	explicit TStr(const PSIn& SIn){ int SInLen=SIn->Len(); RStr=new TRStr(SInLen); RStr->MkRef(); SIn->GetBf(CStr(), SInLen); Optimize(); }
	~TStr(){ RStr->UnRef(); }
	explicit TStr(TSIn& SIn, const bool& IsSmall=false): RStr(new TRStr(SIn, IsSmall)){ RStr->MkRef(); Optimize(); }
	void Load(TSIn& SIn, const bool& IsSmall=false){ *this=TStr(SIn, IsSmall); }
	void Save(TSOut& SOut, const bool& IsSmall=false) const { RStr->Save(SOut, IsSmall); }
	void LoadXml(const PXmlTok& XmlTok, const TStr& Nm);
	void SaveXml(TSOut& SOut, const TStr& Nm) const;

	TStr& operator=(const TStr& Str){ if (this!=&Str) { RStr->UnRef(); RStr=Str.RStr; RStr->MkRef(); } return *this; }
	TStr& operator=(const TChA& ChA){ RStr->UnRef(); RStr=GetRStr(ChA.CStr()); RStr->MkRef(); return *this; }
	TStr& operator=(const char* CStr){ RStr->UnRef(); RStr=GetRStr(CStr); RStr->MkRef(); return *this; }
	TStr& operator=(const char& Ch){ RStr->UnRef(); RStr=GetChStr(Ch).RStr; RStr->MkRef(); return *this; }
	TStr& operator+=(const TStr& Str){ TRStr* NewRStr=new TRStr(RStr->CStr(), Str.RStr->CStr()); RStr->UnRef(); RStr=NewRStr; RStr->MkRef();Optimize(); return *this; }
	TStr& operator+=(const char* CStr){ TRStr* NewRStr=new TRStr(RStr->CStr(), CStr); RStr->UnRef(); RStr=NewRStr; RStr->MkRef(); Optimize(); return *this; }
	bool operator==(const TStr& Str) const { return (RStr==Str.RStr)||(strcmp(RStr->CStr(), Str.RStr->CStr())==0); }
	bool operator==(const char* CStr) const { return strcmp(RStr->CStr(), CStr)==0; }
	//bool operator!=(const TStr& Str) const {
	//return strcmp(RStr->CStr(), Str.RStr->CStr())!=0; }
	bool operator!=(const char* CStr) const { return strcmp(RStr->CStr(), CStr)!=0; }
	bool operator<(const TStr& Str) const { return strcmp(RStr->CStr(), Str.RStr->CStr())<0; }
	...
	void PutCh(const int& ChN, const char& Ch){ TRStr* NewRStr=new TRStr(RStr->CStr()); RStr->UnRef(); RStr=NewRStr; RStr->MkRef(); RStr->PutCh(ChN, Ch); Optimize(); }
	...
	bool IsPrefix(const TStr& Str) const { return GetSubStr(0, Str.Len()-1)==Str; }
	bool IsSuffix(const TStr& Str) const { return GetSubStr(Len()-Str.Len(), Len()-1)==Str; }
	...
	TStr Reverse() const { TChA ChA(*this); ChA.Reverse(); return ChA; }
	...
	int GetInt(const int& DfVal) const { int Val; if (IsInt(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	uint GetUInt(const uint& DfVal) const { uint Val; if (IsUInt(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	int64 GetInt64() const {int64 Val; IAssert(IsInt64(false, 0, 0, Val)); return Val; }
	int64 GetInt64(const int64& DfVal) const {int64 Val; if (IsInt64(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	uint64 GetUInt64() const {uint64 Val; IAssert(IsUInt64(false, 0, 0, Val)); return Val; }
	uint64 GetUInt64(const uint64& DfVal) const {uint64 Val; if (IsUInt64(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	int GetHexInt() const { int Val; IAssert(IsHexInt(false, 0, 0, Val)); return Val; }
	int GetHexInt(const int& DfVal) const {int Val; if (IsHexInt(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	int64 GetHexInt64() const { int64 Val; IAssert(IsHexInt64(false, 0, 0, Val)); return Val; }
	int64 GetHexInt64(const int64& DfVal) const { int64 Val; if (IsHexInt64(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	double GetFlt() const {double Val; IAssert(IsFlt(false, 0, 0, Val)); return Val; }
	double GetFlt(const double& DfVal) const {double Val; if (IsFlt(false, 0, 0, Val)){ return Val; } else { return DfVal; }}
	...
	static TStr LoadTxt(const PSIn& SIn){ return TStr(SIn); }
	static TStr LoadTxt(const TStr& FNm){ PSIn SIn=TFIn::New(FNm); return LoadTxt(SIn); }
	void SaveTxt(const PSOut& SOut) const { SOut->SaveBf(CStr(), Len()); }
	void SaveTxt(const TStr& FNm) const { PSOut SOut=TFOut::New(FNm); SaveTxt(SOut); }
	...
	char* GetCStr() const { char* Bf=new char[Len()+1]; strcpy(Bf, CStr()); return Bf; }
...
public:
	...
	static TStr GetStr(const TInt& UInt){ return GetStr(UInt.Val); }
	static TStr GetStr(const uint& Val, const char* FmtStr);
	static TStr GetStr(const uint& Val, const TStr& FmtStr){ return GetStr(Val, FmtStr.CStr()); }
	...
	static TStr GetMegaStr(const uint& Val){
		if (Val>100*1000000)return GetStr(Val/1000000)+"M";
		else if (Val>1000000)return GetStr(Val/1000000)+"."+GetStr((Val%1000000)/100000)+"M";
		else return GetKiloStr(Val);
	}
...
public:
  ...
  int GetPrimHashCd() const { int Expn; return int((frexp(Val, &Expn)-0.5)*double(TInt::Mx)); }
  int GetSecHashCd() const { int Expn; frexp(Val, &Expn); return Expn; }
};
