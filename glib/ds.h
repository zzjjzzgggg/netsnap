...
public:
	...
	bool operator ()(const TPair<TVal1, TVal2>& P1,	const TPair<TVal1, TVal2>& P2) const {
		if (IsAsc) return P1.Val2 < P2.Val2;
		else return P2.Val2 < P1.Val2;
	}
};
...
public:
	...
	bool operator ==(const TTuple& Tup) const {
		...
		for (int i = 0; i < Len(); i++)	if (ValV[i] != Tup[i]) return false;
		return true;
	}
	bool operator <(const TTuple& Tup) const {
		if (Len() == Tup.Len()) {
			for (int i = 0; i < Len(); i++) {
				if (ValV[i] < Tup[i]) return true;
				else if (ValV[i] > Tup[i]) return false;
			}
			return false;
	...
	int GetPrimHashCd() const {
		uint HashCd = 5381;
		for (int i = 0; i < Len(); i++) HashCd = ((HashCd<<13)+HashCd)+ValV[i].GetPrimHashCd();
		return int(HashCd & 0x7fffffff);
	}
	int GetSecHashCd() const {
		uint HashCd = 5381;
		for (int i = 0; i < Len(); i++) HashCd=((HashCd<<19)+HashCd)+ValV[i].GetSecHashCd();
		return int(HashCd & 0x7fffffff);
	}
	TStr GetStr() const {
		TChA ValsStr;
...
public:
	...
	bool operator ()(const TPair<TVal1, TVal2>& P1,	const TPair<TVal1, TVal2>& P2) const {
		if (IsAsc) return P1.Dat < P2.Dat;
		else return P2.Dat < P1.Dat;
	}
};
...
TStr TVec<TVal>::GetXOutOfBoundsErrMsg(const int& ValN) const {
	return TStr::Fmt("ArrayIndexOutOfBounds. Index: %d, Vals: %d, MaxVals: %d. Type: %s", ValN, Vals, MxVals, GetTypeNm(*this).CStr());
...
int TVec<TVal>::AddBackSorted(const TVal& Val, const bool& Asc) {
	...
	Add();
	int ValN = Vals - 2;
	while ((ValN >= 0) && ((Asc && (Val < ValT[ValN])) || (!Asc && (Val
			> ValT[ValN])))) {
...
template<class TVal>
void TVec<TVal>::BSort(const int& MnLValN, const int& MxRValN, const bool& Asc) {
...
template<class TVal>
void TVec<TVal>::ISort(const int& MnLValN, const int& MxRValN, const bool& Asc) {
	if (MnLValN < MxRValN) {
...
template<class TVal>
int TVec<TVal>::Partition(const int& MnLValN, const int& MxRValN,	const bool& Asc) {
...
template<class TVal>
void TVec<TVal>::QSort(const int& MnLValN, const int& MxRValN, const bool& Asc) {
	if (MnLValN < MxRValN) {
...
public:
	...
	TQQueue(const TQQueue& Queue) :
		MxLast(Queue.MxLast), MxLen(Queue.MxLen), First(Queue.First),
				Last(Queue.Last), ValV(Queue.ValV) {
	}
...
public:
	...
	TVal Val;
public:
	TOLstNd():HPrevNd(NULL), HNextNd(NULL), VPrevNd(NULL), VNextNd(NULL), Val(){}
	TOLstNd(TOLstNd* _PrevNd, TOLstNd* _NextNd, const TVal& _Val, const bool& isHoriz){
		Prev(isHoriz)=_PrevNd;Next(isHoriz)=_NextNd; Val=_Val;
	}
	TOLstNd(const TOLstNd&);
	TOLstNd& operator=(const TOLstNd&);
	TOLstNd*& Prev(const bool& isHoriz) {
		Assert(this != NULL);
		if(isHoriz)return HPrevNd;
		else return VPrevNd;
	}
	TOLstNd*& Next(const bool& isHoriz) {
		Assert(this != NULL);
		if(isHoriz) return HNextNd;
		else return VNextNd;
	}
	TVal& GetVal() {Assert(this != NULL);	return Val;	}
};
...
template<class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::Ins(const POLstNd& Nd, const TVal& Val) {// insert after Nd
	if (Nd == NULL) return AddFront(Val);
	else if (Nd->Next(isHoriz) == NULL) return AddBack(Val);
	else {
		POLstNd NewNd = new TOLstNd<TVal>(Nd, Nd->Next(isHoriz), Val, isHoriz);
...
template<class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddFrontSorted(const TVal& Val, const bool& Asc) {
	if (FirstNd == NULL) return Ins(FirstNd, Val);
	else {
		POLstNd Nd = FirstNd;
		while ((Nd != NULL) && ((Asc && (Val > Nd->Val)) || (!Asc && (Val < Nd->Val)))) Nd = Nd->Next(isHoriz);
		if (Nd == NULL) return Ins(LastNd, Val);
		else return Ins(Nd->Prev(isHoriz), Val);
	}
}
template<class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddBackSorted(const TVal& Val, const bool& Asc) {
	POLstNd Nd = LastNd;
	while ((Nd != NULL) && ((Asc && (Val < Nd->Val)) || (!Asc && (Val > Nd->Val)))) Nd = Nd->Prev(isHoriz);
	return Ins(Nd, Val);
}
...
template<class TVal>
void TOLst<TVal>::AddBack(const TInt& x, const TInt& y, const TVal& Val){
	Assert(x>=0&&x<nh&&y>=0&&y<nv);
	HLsts[x].AddBack(Val);
	VLsts[y].AddBack(Val);
	Nds++;
...
template<class TVal>
void TOLst<TVal>::AddFrontSorted(const TInt& x, const TInt& y, const TVal& Val, const bool& Asc){
	Assert(x>=0&&x<nh&&y>=0&&y<nv);
...
template<class TVal>
void TOLst<TVal>::AddBackSorted(const TInt& x, const TInt& y, const TVal& Val, const bool& Asc){
	Assert(x>=0&&x<nh&&y>=0&&y<nv);
