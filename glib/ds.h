...
class TPair {
   ...
   public:
    ...
    int GetPrimHashCd() const {
        return Val1.GetPrimHashCd() + Val2.GetPrimHashCd();
    }  // J: terrible hash function!
    int GetSecHashCd() const {
        return Val1.GetSecHashCd() + Val2.GetSecHashCd();
    }  // J: terrible hash function!
...
class TCmpPairByVal2 {
   ...
   public:
    ...
    bool operator()(const TPair<TVal1, TVal2>& P1,
                    const TPair<TVal1, TVal2>& P2) const {
        if (IsAsc)
...
class TTriple {
   ...
   public:
    ...
    TTriple(const TVal1& _Val1, const TVal2& _Val2,
            const TVal3& _Val3)
    ...
    bool operator==(const TTriple& Triple) const {
        return Val1 == Triple.Val1 && Val2 == Triple.Val2 &&
               Val3 == Triple.Val3;
    }
    bool operator<(const TTriple& Triple) const {
        return Val1 < Triple.Val1 ||
...
class TQuad {
   ...
   public:
    TQuad() : Val1(), Val2(), Val3(), Val4() {}
    TQuad(const TQuad& Quad)
        : Val1(Quad.Val1),
          Val2(Quad.Val2),
          Val3(Quad.Val3),
          Val4(Quad.Val4) {}
    TQuad(const TVal1& _Val1, const TVal2& _Val2, const TVal3& _Val3,
          const TVal4& _Val4)
    ...
    bool operator==(const TQuad& Quad) const {
        return (Val1 == Quad.Val1) && (Val2 == Quad.Val2) &&
               (Val3 == Quad.Val3) && (Val4 == Quad.Val4);
    }
    bool operator<(const TQuad& Quad) const {
        return (Val1 < Quad.Val1) ||
               ((Val1 == Quad.Val1) && (Val2 < Quad.Val2)) ||
               ((Val1 == Quad.Val1) && (Val2 == Quad.Val2) &&
                (Val3 < Quad.Val3)) ||
               ((Val1 == Quad.Val1) && (Val2 == Quad.Val2) &&
                (Val3 == Quad.Val3) && (Val4 < Quad.Val4));
    ...
    int GetPrimHashCd() const {
        return Val1.GetPrimHashCd() + Val2.GetPrimHashCd() +
               Val3.GetPrimHashCd() + Val4.GetPrimHashCd();
    }
    int GetSecHashCd() const {
        return Val1.GetSecHashCd() + Val2.GetSecHashCd() +
               Val3.GetSecHashCd() + Val4.GetSecHashCd();
    }

    void GetVal(TVal1& _Val1, TVal2& _Val2, TVal3& _Val3,
                TVal4& _Val4) const {
        _Val1 = Val1;
        _Val2 = Val2;
        _Val3 = Val3;
...
class TTuple {
   ...
   public:
    ...
    //  for (int i=0; i<Len(); i++) { HashCd +=
    //  ValV[i].GetPrimHashCd(); } return HashCd; }
    // int GetSecHashCd() const { int HashCd=0;
    //  for (int i=0; i<Len(); i++) { HashCd +=
    //  ValV[i].GetSecHashCd(); } return HashCd; }
    int GetPrimHashCd() const {
        uint HashCd = 5381;
        for (int i = 0; i < Len(); i++)
            HashCd =
                ((HashCd << 13) + HashCd) + ValV[i].GetPrimHashCd();
        return int(HashCd & 0x7fffffff);
    }
    int GetSecHashCd() const {
        uint HashCd = 5381;
        for (int i = 0; i < Len(); i++)
            HashCd =
                ((HashCd << 19) + HashCd) + ValV[i].GetSecHashCd();
        return int(HashCd & 0x7fffffff);
    }
    TStr GetStr() const {
        TChA ValsStr;
...
class TVec {
   ...
   protected:
    int MxVals;  // if MxVals==-1, then ValT is not owned by us, we
                 // don't free it!
    int Vals;    // length of the vector.
    TVal* ValT;
   ...
   public:
    ...
    explicit TVec(TSIn& SIn) : MxVals(0), Vals(0), ValT(NULL) {
        Load(SIn);
    }
    void Load(TSIn& SIn);
    void Save(TSOut& SOut) const;
    ...
    bool operator<(const TVec<TVal>& Vec) const;
    const TVal& operator[](const int& ValN) const {
        AssertR(0 <= ValN && ValN < Vals,
                GetXOutOfBoundsErrMsg(ValN));
        return ValT[ValN];
    }
    ...
    int GetMemUsed() const {
        return 2 * sizeof(int) + sizeof(TVal*) +
               MxVals * sizeof(TVal);
    }
    ...
    int Add(const TVal& Val, const int& ResizeLen) {
        Assert(MxVals != -1);
        if (Vals == MxVals) Resize(MxVals + ResizeLen);
        ValT[Vals] = Val;
        return Vals++;
    }
    int AddV(const TVec<TVal>& ValV);
    int AddSorted(const TVal& Val, const bool& Asc = true,
                  const int& _MxVals = -1);
    ...
    void GetSubValV(const int& BValN, const int& EValN,
                    TVec<TVal>& ValV) const;
    ...
    const TVal& GetRnd(TRnd& Rnd = TInt::Rnd) const {
        return ValT[Rnd.GetUniDevInt(Vals)];
    }
    ...
    void BSort(const int& MnLValN, const int& MxRValN,
               const bool& Asc);
    void ISort(const int& MnLValN, const int& MxRValN,
               const bool& Asc);
    int Partition(const int& MnLValN, const int& MxRValN,
                  const bool& Asc);
    void QSort(const int& MnLValN, const int& MxRValN,
               const bool& Asc);
    void Sort(const bool& Asc = true);
    ...
    void PushHeap(const TVal& Val) {
        PushHeap(Val, TLss<TVal>());
    }  // add element to the heap
    const TVal& TopHeap() const {
        return ValT[0];
    }  // get largest element
    TVal PopHeap() {
        return PopHeap(TLss<TVal>());
    ...
    template <class TCmp>
    void PushHeap(const int& First, int HoleIdx, const int& Top,
                  TVal Val, const TCmp& Cmp) {
    ...
    template <class TCmp>
    void AdjustHeap(const int& First, int HoleIdx, const int& Len,
                    TVal Val, const TCmp& Cmp) {
        const int Top = HoleIdx;
        int Right = 2 * HoleIdx + 2;
        while (Right < Len) {
            if (Cmp(ValT[First + Right], ValT[First + Right - 1]))
                Right--;
    ...
    void MakeHeap(const int& First, const int& Len,
        ...
        int Parent = (Len - 2) / 2;
        while (true) {
            AdjustHeap(First, Parent, Len, ValT[First + Parent],
                       Cmp);
            if (Parent == 0) return;
            Parent--;
        }
    ...
    template <class TCmp>
    static TIter GetPivotValNCmp(const TIter& BI, const TIter& EI,
                                 const TCmp& Cmp) {
    ...
    template <class TCmp>
    static TIter PartitionCmp(TIter BI, TIter EI, const TVal Pivot,
                              const TCmp& Cmp) {
        forever {
    ...
    static void QSortCmp(TIter BI, TIter EI, const TCmp& Cmp) {
        if (BI + 1 < EI) {
            if (EI - BI < 20)
                ...
                TIter Split = PartitionCmp(
                    BI, EI, *GetPivotValNCmp(BI, EI, Cmp), Cmp);
    ...
    int SearchVForw(const TVec<TVal>& ValV,
                    const int& BValN = 0) const;
    ...
    static TVec<TVal> GetV(const TVal& Val1, const TVal& Val2,
                           const TVal& Val3, const TVal& Val4) {
        TVec<TVal> V(4, 0);
        V.Add(Val1);
        V.Add(Val2);
        V.Add(Val3);
    ...
    static TVec<TVal> GetV(const TVal& Val1, const TVal& Val2,
                           const TVal& Val3, const TVal& Val4,
                           const TVal& Val5, const TVal& Val6) {
        TVec<TVal> V(6, 0);
        V.Add(Val1);
        V.Add(Val2);
        V.Add(Val3);
    ...
    static TVec<TVal> GetV(const TVal& Val1, const TVal& Val2,
                           const TVal& Val3, const TVal& Val4,
                           const TVal& Val5, const TVal& Val6,
                           const TVal& Val7, const TVal& Val8) {
        TVec<TVal> V(8, 0);
        V.Add(Val1);
        V.Add(Val2);
        V.Add(Val3);
    ...
    //  TVec<TVal> V; while(*ValPt!=EndVal){V.Add(*ValPt); ValPt++; }
    //  return V; }
};

template <class TVal>
void TVec<TVal>::Resize(const int& _MxVals) {
    IAssertR(MxVals != -1, TStr::Fmt("Can not resize buffer. %s",
                                     GetTypeNm(*this).CStr())
                               .CStr());
    if (_MxVals == -1) {
        if (Vals == 0)
    ...
    if (ValT == NULL) {
        try {
            ...
            EFailR(
                TStr::Fmt("TVec::Resize: %s, Vals:%d, MxVals:%d, "
                          "_MxVals:%d, Type:%s",
                          Ex.what(), Vals, MxVals, _MxVals,
                          GetTypeNm(*this).CStr()));
        }
    } else {
        // if (Vals > 1000000) {
...
TStr TVec<TVal>::GetXOutOfBoundsErrMsg(const int& ValN) const {
    return TStr::Fmt(
        "ArrayIndexOutOfBounds. Index: %d, Vals: %d, MaxVals: %d. "
        "Type: %s",
        ValN, Vals, MxVals, GetTypeNm(*this).CStr());
    //	return TStr() + "Index:" + TInt::GetStr(ValN) + " Vals:" +
    //TInt::GetStr(Vals) + " MxVals:" + TInt::GetStr(MxVals) +"
    //Type:" + GetTypeNm(*this);
}
...
void TVec<TVal>::Load(TSIn& SIn) {
    ...
    if (MxVals == 0)
        ...
        ValT = new TVal[MxVals];
    for (int ValN = 0; ValN < Vals; ValN++) ValT[ValN] = TVal(SIn);
}
...
int TVec<TVal>::AddV(const TVec<TVal>& ValV) {
    Assert(MxVals != -1);
    for (int ValN = 0; ValN < ValV.Vals; ValN++) {
        Add(ValV[ValN]);
    }
    return Len();
}
...
template <class TVal>
int TVec<TVal>::AddSorted(const TVal& Val, const bool& Asc,
                          const int& _MxVals) {
    Assert(MxVals != -1);
    int ValN = Add(Val);
    if (Asc) {
...
int TVec<TVal>::AddBackSorted(const TVal& Val, const bool& Asc) {
    ...
    Add();
    int ValN = Vals - 2;
    while ((ValN >= 0) && ((Asc && (Val < ValT[ValN])) ||
                           (!Asc && (Val > ValT[ValN])))) {
...
template <class TVal>
void TVec<TVal>::GetSubValV(const int& _BValN, const int& _EValN,
                            TVec<TVal>& SubValV) const {
...
void TVec<TVal>::Ins(const int& ValN, const TVal& Val) {
    ...
    for (int MValN = Vals - 2; MValN >= ValN; MValN--) {
...
template <class TVal>
void TVec<TVal>::BSort(const int& MnLValN, const int& MxRValN,
                       const bool& Asc) {
...
template <class TVal>
void TVec<TVal>::ISort(const int& MnLValN, const int& MxRValN,
                       const bool& Asc) {
    if (MnLValN < MxRValN) {
...
template <class TVal>
int TVec<TVal>::Partition(const int& MnLValN, const int& MxRValN,
                          const bool& Asc) {
...
template <class TVal>
void TVec<TVal>::QSort(const int& MnLValN, const int& MxRValN,
                       const bool& Asc) {
    if (MnLValN < MxRValN) {
...
void TVec<TVal>::Merge() {
    Assert(MxVals != -1);
    TVec<TVal> SortedVec(*this);
    SortedVec.Sort();
    Clr();
    for (int ValN = 0; ValN < SortedVec.Len(); ValN++) {
        if ((ValN == 0) ||
            (SortedVec[ValN - 1] != SortedVec[ValN])) {
            Add(SortedVec[ValN]);
        }
    }
}
...
bool TVec<TVal>::NextPerm() {
    ...
    for (;;) {
        ...
        int Next1 = Next;
        if (GetVal(--Next) <
            GetVal(Next1)) {  // swap with rightmost element that's
                              // smaller, flip suffix
            int Mid = Last;
...
void TVec<TVal>::Union(const TVec<TVal>& ValV,
    ...
    for (int RestValN2 = ValN2; RestValN2 < ValV.Len();
         RestValN2++) {
...
int TVec<TVal>::IntrsLen(const TVec<TVal>& ValV) const {
    ...
    while (ValN1 < Len() && ValN2 < ValV.Len()) {
        ...
        while (ValN2 < ValV.Len() && Val1 != ValV.GetVal(ValN2))
            ValN2++;
        if (ValN2 < ValV.Len() && Val1 == ValV.GetVal(ValN2)) Cnt++;
        ValN2 = 0;
        ValN1++;
    }
    return Cnt;
...
class TVecPool {
   ...
   public:
    ...
    ::TSize GetMemUsed() const {
        return sizeof(TCRef) + sizeof(TBool) + 3 * sizeof(TSize) +
               sizeof(TVal*) + MxVals * sizeof(TVal);
    }
    ...
    // shuffles all the order of elements in the Pool(does not
    // respect
    // vector boundaries)
...
template <class TVal>
TVecPool<TVal>::TVecPool(TSIn& SIn) : FastCopy(SIn) {
...
void TVecPool<TVal>::CompactPool(const TVal& DelVal) {
    ...
    for (int vid = 1; vid < IdToOffV.Len(); vid++) {
        ...
        memcpy(ValV - TotalDel, ValV,
               sizeof(TVal) * Len);  // move data
        TotalDel += NDel;
    }
...
void TVecPool<TVal>::ShuffleAll(TRnd& Rnd) {
    for (::TSize n = Vals - 1; n > 0; n--) {
        const ::TSize k =
            ::TSize(((uint64(Rnd.GetUniDevInt()) << 32) |
                     uint64(Rnd.GetUniDevInt())) %
                    (n + 1));
...
class TVVec {
   ...
   public:
    ...
    explicit TVVec(const TVec<TVal>& _ValV, const int& _XDim,
                   const int& _YDim)
        : XDim(_XDim), YDim(_YDim), ValV(_ValV) {
    ...
    bool operator==(const TVVec& Vec) const {
        return (XDim == Vec.XDim) && (YDim == Vec.YDim) &&
               (ValV == Vec.ValV);
    }
    ...
    TVal& operator()(const int& X, const int& Y) { return At(X, Y); }
...
class TVVVec {
   ...
   public:
    ...
    const TVal& operator()(const int& X, const int& Y,
                           const int& Z) const {
        return At(X, Y, Z);
    }
    int GetXDim() const { return XDim; }
...
class TQQueue {
   ...
   public:
    ...
    void GetSubValV(const int& _BValN, const int& _EValN,
                    TVec<TVal>& SubValV) const {
...
TLstNd<TVal>* TLst<TVal>::AddFrontSorted(const TVal& Val,
    ...
    if (FirstNd == NULL)
        ...
        while (Nd != NULL &&
            ...
            Nd = Nd->NextNd;
        if (Nd == NULL)
            return Ins(LastNd, Val);
        else
            return Ins(Nd->PrevNd, Val);
    }
}
...
void TLst<TVal>::Del(const PLstNd& Nd) {
    Assert(Nd != NULL);
    if (Nd->PrevNd == NULL)
        FirstNd = Nd->NextNd;  // delete head
    else
        Nd->PrevNd->NextNd = Nd->NextNd;
    if (Nd->NextNd == NULL)
        LastNd = Nd->PrevNd;  // delete tail
    else
        Nd->NextNd->PrevNd = Nd->PrevNd;
    Nds--;
    delete Nd;
...
class TOLstNd {
   public:
    TOLstNd *HPrevNd, *HNextNd, *VPrevNd, *VNextNd;
    TVal Val;
   ...
   public:
    TOLstNd()
        : HPrevNd(NULL),
          ...
          VNextNd(NULL),
          Val() {}
    TOLstNd(TOLstNd* _PrevNd, TOLstNd* _NextNd, const TVal& _Val,
            const bool& isHoriz) {
        Prev(isHoriz) = _PrevNd;
        Next(isHoriz) = _NextNd;
        Val = _Val;
    }
    TOLstNd(const TOLstNd&);
    TOLstNd& operator=(const TOLstNd&);
    TOLstNd*& Prev(const bool& isHoriz) {
        Assert(this != NULL);
        if (isHoriz)
            return HPrevNd;
        else
    ...
    TOLstNd*& Next(const bool& isHoriz) {
        Assert(this != NULL);
        if (isHoriz)
            return HNextNd;
        else
...
/**
 * horiz and vertical header
 */
template <class TVal>
class TOLstHdr {
   public:
    typedef TOLstNd<TVal>* POLstNd;
    POLstNd FirstNd, LastNd;
    int Nds;
    bool isHoriz;

   public:
    TOLstHdr()
        : FirstNd(NULL), LastNd(NULL), Nds(0), isHoriz(true) {}
    ~TOLstHdr() { Clr(); }
    void Clr() {
        POLstNd p;
        while (FirstNd != NULL) {
            p = FirstNd;
            FirstNd = FirstNd->Next(isHoriz);
            delete p;
        }
        Nds = 0;
        FirstNd = LastNd = NULL;
    ...
    POLstNd AddFrontSorted(const TVal& Val, const bool& Asc = true);
    POLstNd AddBackSorted(const TVal& Val, const bool& Asc = true);
};
template <class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddFront(const TVal& Val) {
    POLstNd Nd = new TOLstNd<TVal>(NULL, FirstNd, Val, isHoriz);
    if (FirstNd != NULL) {
        FirstNd->Prev(isHoriz) = Nd;
        FirstNd = Nd;
    } else {
        FirstNd = Nd;
        LastNd = Nd;
...
template <class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddBack(const TVal& Val) {
    POLstNd Nd = new TOLstNd<TVal>(LastNd, NULL, Val, isHoriz);
    if (LastNd != NULL) {
        LastNd->Next(isHoriz) = Nd;
        LastNd = Nd;
    } else {
        FirstNd = Nd;
        LastNd = Nd;
...
template <class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::Ins(
    const POLstNd& Nd, const TVal& Val) {  // insert after Nd
    if (Nd == NULL)
        return AddFront(Val);
    else if (Nd->Next(isHoriz) == NULL)
        return AddBack(Val);
    else {
        POLstNd NewNd =
            new TOLstNd<TVal>(Nd, Nd->Next(isHoriz), Val, isHoriz);
        Nd->Next(isHoriz) = NewNd;
        NewNd->Next(isHoriz)->Prev(isHoriz) = NewNd;
        Nds++;
        return NewNd;
    }
}
template <class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddFrontSorted(const TVal& Val,
                                              const bool& Asc) {
    if (FirstNd == NULL)
        return Ins(FirstNd, Val);
    else {
        POLstNd Nd = FirstNd;
        while ((Nd != NULL) && ((Asc && (Val > Nd->Val)) ||
                                (!Asc && (Val < Nd->Val))))
            Nd = Nd->Next(isHoriz);
        if (Nd == NULL)
            return Ins(LastNd, Val);
        else
            return Ins(Nd->Prev(isHoriz), Val);
    }
}
template <class TVal>
TOLstNd<TVal>* TOLstHdr<TVal>::AddBackSorted(const TVal& Val,
                                             const bool& Asc) {
    POLstNd Nd = LastNd;
    while ((Nd != NULL) &&
           ((Asc && (Val < Nd->Val)) || (!Asc && (Val > Nd->Val))))
...
class TOLst {
   public:
    typedef TOLstNd<TVal>* POLstNd;
    typedef TOLstHdr<TVal>* POLstHdr;

   private:
    int nh, nv, Nds;
    POLstHdr HLsts, VLsts;

   public:
    TOLst(int nh, int nv) : nh(nh), nv(nv), Nds(0) {
        HLsts = new TOLstHdr<TVal>[nh];
    ...
    void AddFront(const TInt& x, const TInt& y, const TVal& Val);
    void AddBack(const TInt& x, const TInt& y, const TVal& Val);
    void AddFrontSorted(const TInt& x, const TInt& y,
                        const TVal& Val, const bool& Asc = true);
    void AddBackSorted(const TInt& x, const TInt& y, const TVal& Val,
                       const bool& Asc = true);
    const int LenH() { return nh; };
    const int LenV() { return nv; };
    const int Len() { return Nds; };

    POLstHdr GetHLst(int i) {
        Assert(i >= 0 && i < nh);
        return &HLsts[i];
    }
    POLstHdr GetVLst(int i) {
        Assert(i >= 0 && i < nv);
        return &VLsts[i];
    }
};
template <class TVal>
void TOLst<TVal>::AddFront(const TInt& x, const TInt& y,
                           const TVal& Val) {
    Assert(x >= 0 && x < nh && y >= 0 && y < nv);
    HLsts[x].AddFront(Val);
    VLsts[y].AddFront(Val);
    Nds++;
}
template <class TVal>
void TOLst<TVal>::AddBack(const TInt& x, const TInt& y,
                          const TVal& Val) {
    Assert(x >= 0 && x < nh && y >= 0 && y < nv);
    HLsts[x].AddBack(Val);
    VLsts[y].AddBack(Val);
...
template <class TVal>
void TOLst<TVal>::AddFrontSorted(const TInt& x, const TInt& y,
                                 const TVal& Val, const bool& Asc) {
    Assert(x >= 0 && x < nh && y >= 0 && y < nv);
    HLsts[x].AddFrontSorted(Val, Asc);
    VLsts[y].AddFrontSorted(Val, Asc);
    Nds++;
}
template <class TVal>
void TOLst<TVal>::AddBackSorted(const TInt& x, const TInt& y,
                                const TVal& Val, const bool& Asc) {
    Assert(x >= 0 && x < nh && y >= 0 && y < nv);
    HLsts[x].AddBackSorted(Val, Asc);
    VLsts[y].AddBackSorted(Val, Asc);
    Nds++;
}
